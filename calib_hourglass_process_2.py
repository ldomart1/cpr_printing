...
# -*- coding: utf-8 -*-
"""
offline_run_checkerboard_hourglass_error_analysis.py
...
Usage example:
  python3 offline_run_checkerboard_hourglass_error_analysis.py \
...
CIRCLE_RECORDED_PHASES_ORDER = ["q1_pull", "q2_release", "q3_pull", "q4_release"]
CIRCLE_RECORDED_PHASES = set(CIRCLE_RECORDED_PHASES_ORDER)

LEGACY_STAR_RECORDED_PHASES_ORDER = ["right", "left"]
...
\
def is_recorded_phase(phase: Optional[str]) -> bool:
    phase_name = str(phase).strip()
    if phase_name in HOURGLASS_RECORDED_PHASES or phase_name in CIRCLE_RECORDED_PHASES or phase_name in LEGACY_STAR_RECORDED_PHASES:
        return True
    if phase_name.endswith("_start"):
        base = phase_name[:-6]
        return (
            base in HOURGLASS_RECORDED_PHASES
            or base in CIRCLE_RECORDED_PHASES
            or base in LEGACY_STAR_RECORDED_PHASES
        )
    return False
...
def make_arc_points(
    ...
    thetas = np.deg2rad(np.linspace(float(theta_start_deg), float(theta_end_deg), int(max(2, n)), endpoint=True))
    x = float(center_x) + float(radius) * np.cos(thetas)
    z = float(center_z) + float(radius) * np.sin(thetas)
...
def make_line_points(p0: np.ndarray, p1: np.ndarray, n: int) -> np.ndarray:
    ...
    tt = np.linspace(0.0, 1.0, int(max(2, n)), endpoint=True)[:, None]
...
def build_hourglass_tip_rows(
    ...
    if float(arc_vertical_gap_mm) < 0.0:
        raise ValueError("arc_vertical_gap_mm must be non-negative.")
    if float(middle_gap_mm) < 0.0:
        raise ValueError("middle_gap_mm must be non-negative.")
    if float(middle_gap_mm) >= 2.0 * float(radius):
        raise ValueError("middle_gap_mm must be smaller than the circle diameter.")

    phi_deg = float(max(0.0, min(80.0, float(arc_overtravel_deg))))
    n_arc = int(max(2, samples_per_arc))
    n_diag = int(max(2, samples_per_diagonal))
    ...
    for seg_idx, (phase_name, tip_xz_points) in enumerate(segments):
        ...
        for i, pt in enumerate(tip_xz_points):
            if seg_idx > 0 and i == 0:
                continue
            phase = f"{phase_name}_start" if (seg_idx == 0 and i == 0) else str(phase_name)
            rows.append({
                "phase": phase,
                "tip_x": float(pt[0]),
                "tip_z": float(pt[1]),
    ...
    if bool(final_recenter) and rows:
        start_tip = np.array([rows[0]["tip_x"], rows[0]["tip_z"]], dtype=np.float64)
        end_tip = np.array([rows[-1]["tip_x"], rows[-1]["tip_z"]], dtype=np.float64)
        if float(np.linalg.norm(end_tip - start_tip)) > 1e-9:
            rows.append({
                "phase": "final_recenter",
                "tip_x": float(start_tip[0]),
                "tip_z": float(start_tip[1]),
            })
    return rows


def build_hourglass_tip_lookup(
    center_x: float,
    center_z: float,
    arc_vertical_gap_mm: float,
    circle_diameter_mm: float,
    ...
    rows = build_hourglass_tip_rows(
        ...
        arc_vertical_gap_mm=float(arc_vertical_gap_mm),
        circle_diameter_mm=float(circle_diameter_mm),
        middle_gap_mm=float(middle_gap_mm),
    ...
    sample_idx = 0
    shape_move_counter = 0
    capture_every_n_shape_moves = max(1, int(capture_every_n_shape_moves))

    if bool(capture_at_start) and rows:
        sample_idx += 1
        lookup[sample_idx] = {
            "desired_tip_x_mm": float(rows[0]["tip_x"]),
            "desired_tip_z_mm": float(rows[0]["tip_z"]),
        }

    for row in rows[1:]:
        phase = str(row.get("phase", "")).strip()
        if not is_recorded_phase(phase):
            continue
        shape_move_counter += 1
        if (shape_move_counter % capture_every_n_shape_moves) != 0:
            continue
        sample_idx += 1
        lookup[sample_idx] = {
...
def build_tip_lookup_from_planned_csv(
...
def resolve_desired_tip_lookup(project_dir: Path, args) -> Dict[int, Dict[str, float]]:
    ...
    candidate_csvs.append(Path(project_dir) / "planned_hourglass_command_sequence.csv")
    candidate_csvs.append(Path(project_dir) / "planned_command_sequence.csv")

    for candidate in candidate_csvs:
        try:
            if candidate.exists():
                lookup = build_tip_lookup_from_planned_csv(
                    candidate,
    ...
    print("[INFO] Planned command CSV not found; using ideal hourglass lookup fallback.")
    return build_hourglass_tip_lookup(
...
def _phase_to_segment_label(phase: Optional[str]) -> Optional[str]:
    ...
    for seg in HOURGLASS_RECORDED_PHASES_ORDER:
        if phase_str.startswith(seg):
            return seg
    for seg in CIRCLE_RECORDED_PHASES_ORDER:
        if phase_str.startswith(seg):
            return seg
    for seg in LEGACY_STAR_RECORDED_PHASES_ORDER:
        if phase_str.startswith(seg):
            return seg
    return None


def compute_segment_error_stats(records: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    segment_order = (
        list(HOURGLASS_RECORDED_PHASES_ORDER)
        + list(CIRCLE_RECORDED_PHASES_ORDER)
        + list(LEGACY_STAR_RECORDED_PHASES_ORDER)
    )
    segment_errors: Dict[str, List[float]] = {seg: [] for seg in segment_order}

    for rec in records:
        err = rec.get("error_distance_mm")
        if err is None:
            continue
        segment = _phase_to_segment_label(rec.get("phase"))
        if segment is None:
            continue
        segment_errors.setdefault(segment, []).append(float(err))

    stats: Dict[str, Dict[str, Any]] = {}
    for segment in segment_order:
        values = segment_errors.get(segment, [])
...
def save_desired_vs_actual_plot(
    ...
    if not desired_recs:
        raise RuntimeError("Desired tip X/Z values were unavailable for the desired lookup.")
...
def save_metrics_json(
    ...
    payload = {
        ...
        "settings": {
            ...
            "hourglass_center_x_mm": float(args.hourglass_center_x_mm),
            "hourglass_center_z_mm": float(args.hourglass_center_z_mm),
            "arc_vertical_gap_mm": float(args.arc_vertical_gap_mm),
            "circle_diameter_mm": float(args.circle_diameter_mm),
            "middle_gap_mm": float(args.middle_gap_mm),
            "arc_overtravel_deg": float(args.arc_overtravel_deg),
...
def main():
    ...
    ap.add_argument("--calibration", type=str, default=None,
                    help="Optional compatibility arg from DAQ script; ignored by postprocessing.")
    ...
    ap.add_argument(
        ...
        help="Which tip rows to convert to mm. auto uses selected/CNN when --tip_refiner_model is active, otherwise coarse. Compatibility aliases: --tracked-tip-source, --post-tracked-tip-source, --post_tracked_tip_source.",
    )
    ...
    ap.add_argument("--arc_vertical_gap_mm", type=float, default=HOURGLASS_ARC_VERTICAL_GAP_MM_DEFAULT)
    ap.add_argument("--circle_diameter_mm", type=float, default=HOURGLASS_CIRCLE_DIAMETER_MM_DEFAULT)
    ap.add_argument("--middle_gap_mm", type=float, default=HOURGLASS_MIDDLE_GAP_MM_DEFAULT)
    ap.add_argument("--arc_overtravel_deg", type=float, default=HOURGLASS_ARC_OVERTRAVEL_DEG_DEFAULT)
    ap.add_argument("--samples_per_arc", type=int, default=HOURGLASS_SAMPLES_PER_ARC_DEFAULT,
    ...
    for segment in HOURGLASS_RECORDED_PHASES_ORDER:
        ...
        if int(sstats.get("num_samples", 0)) == 0:
            print(f"  {segment}: no samples")
            continue
        print(
            f"  {segment}: n={int(sstats['num_samples'])}, "
            f"rmse={float(sstats['rmse_mm']):.6f} mm, "
            f"mean={float(sstats['mean_error_mm']):.6f} mm, "
            f"std={float(sstats['std_error_mm']):.6f} mm, "
            f"median={float(sstats['median_error_mm']):.6f} mm, "
            f"min={float(sstats['min_error_mm']):.6f} mm, "
            f"max={float(sstats['max_error_mm']):.6f} mm"
        )
    print("=============================\n")

    print("[DONE] Offline checkerboard hourglass tip error analysis complete.")
