...
class Calibration:
    ...
    phase_models: Optional[Dict[str, Dict[str, Optional[Dict[str, Any]]]]] = None
...
def canonical_motion_phase(phase_name: Optional[str]) -> str:
...
def resolve_motion_phase(
    target_b: float,
    prev_b: Optional[float] = None,
    prev_phase: Optional[str] = None,
    default_phase: str = "pull",
    eps: float = 1e-9,
) -> str:
    if prev_b is None:
        return canonical_motion_phase(prev_phase or default_phase)
    delta = float(target_b) - float(prev_b)
    if delta < -float(eps):
        return "pull"
    if delta > float(eps):
        return "release"
...
def load_calibration(json_path: str) -> Calibration:
    ...
    active_phase_models = raw_phase_models.get(active_phase) if isinstance(raw_phase_models, dict) else None
    ...
    phase_models: Dict[str, Dict[str, Optional[Dict[str, Any]]]] = {}
    if isinstance(raw_phase_models, dict):
        for raw_phase_name, raw_models in raw_phase_models.items():
            ...
            phase_models[phase_key] = {
                "r": _select_named_model(raw_models, "r", selected_fit_model),
                "z": _select_named_model(raw_models, "z", selected_fit_model),
                "offplane_y": phase_y_off_model,
                "offplane_y_extrap": phase_y_off_extrap,
                "tip_angle": _select_named_model(raw_models, "tip_angle", selected_fit_model),
            }
    if "pull" not in phase_models:
        phase_models["pull"] = {
            "r": r_model,
            "z": z_model,
            "offplane_y": y_off_model,
            "offplane_y_extrap": y_off_extrap_model,
            "tip_angle": tip_angle_model,
        }
    if "release" not in phase_models:
        phase_models["release"] = dict(phase_models["pull"])
...
def _phase_model(cal: Calibration, motion_phase: Optional[str], key: str) -> Optional[Dict[str, Any]]:
    phase_key = canonical_motion_phase(motion_phase or cal.active_phase)
    if cal.phase_models and phase_key in cal.phase_models:
        spec = cal.phase_models[phase_key].get(key)
        if spec is not None:
            return spec
    if phase_key == canonical_motion_phase(cal.active_phase):
        if key == "r":
            return cal.r_model
        if key == "z":
            return cal.z_model
        if key == "offplane_y":
            return cal.y_off_model
        if key == "offplane_y_extrap":
            return cal.y_off_extrap_model
        if key == "tip_angle":
            return cal.tip_angle_model
    return None


def eval_r(cal: Calibration, b: Any, motion_phase: Optional[str] = None) -> np.ndarray:
    r_model = _phase_model(cal, motion_phase, "r")
    if r_model is not None:
        return eval_model_spec(r_model, b)
    return poly_eval(cal.pr, b)


def eval_z(cal: Calibration, b: Any, motion_phase: Optional[str] = None) -> np.ndarray:
    z_model = _phase_model(cal, motion_phase, "z")
    if z_model is not None:
        return eval_model_spec(z_model, b)
    return poly_eval(cal.pz, b)


def eval_offplane_y(cal: Calibration, b: Any, motion_phase: Optional[str] = None) -> np.ndarray:
    y_off_model = _phase_model(cal, motion_phase, "offplane_y")
    y_off_extrap_model = _phase_model(cal, motion_phase, "offplane_y_extrap")
    if y_off_model is not None:
        if str(y_off_model.get("model_type", "")).lower() == "pchip":
            values = eval_pchip_with_linear_extrap(y_off_model, y_off_extrap_model, b)
        else:
            values = eval_model_spec(y_off_model, b, default_if_none=0.0)
    else:
...
def eval_tip_angle_deg(cal: Calibration, b: Any, motion_phase: Optional[str] = None) -> np.ndarray:
    tip_angle_model = _phase_model(cal, motion_phase, "tip_angle")
...
def predict_tip_offset_xyz(
    ...
    phase = resolve_motion_phase(
        target_b=float(b),
        prev_b=prev_b,
        prev_phase=prev_phase,
        default_phase=cal.active_phase,
    ) if motion_phase is None else canonical_motion_phase(motion_phase)
    source_phase = canonical_motion_phase(prev_phase or phase)

    r = float(eval_r(cal, b, motion_phase=phase))
    z = float(eval_z(cal, b, motion_phase=phase))
    y_off = float(eval_offplane_y(cal, b, motion_phase=phase))

    if prev_b is not None and phase != source_phase:
        start_b = float(prev_b)
        r += float(eval_r(cal, start_b, motion_phase=source_phase)) - float(eval_r(cal, start_b, motion_phase=phase))
        z += float(eval_z(cal, start_b, motion_phase=source_phase)) - float(eval_z(cal, start_b, motion_phase=phase))
        y_off += float(eval_offplane_y(cal, start_b, motion_phase=source_phase)) - float(eval_offplane_y(cal, start_b, motion_phase=phase))
...
def choose_b_for_target_tip_angle(
    cal: Calibration,
    target_angle_deg: float,
    prev_b: Optional[float] = None,
    prev_phase: Optional[str] = None,
    ...
    if prev_b is None:
        phase = canonical_motion_phase(prev_phase or cal.active_phase)
        return (
            solve_b_for_target_tip_angle(cal, target_angle_deg, search_samples=search_samples, motion_phase=phase),
            phase,
        )

    candidates: List[Tuple[int, float, int, float, str]] = []
    for phase in ("pull", "release"):
        b_val = solve_b_for_target_tip_angle(cal, target_angle_deg, search_samples=search_samples, motion_phase=phase)
        delta = float(b_val) - float(prev_b)
        direction_valid = (phase == "pull" and delta <= 1e-9) or (phase == "release" and delta >= -1e-9)
        phase_bias = 0 if phase == canonical_motion_phase(prev_phase) else 1
        candidates.append((0 if direction_valid else 1, abs(delta), phase_bias, float(b_val), phase))

    candidates.sort(key=lambda item: (item[0], item[1], item[2]))
    _, _, _, b_best, phase_best = candidates[0]
    return float(b_best), str(phase_best)
...
def row_branch_approach_axis(row_num: int, lateral_axis: str) -> np.ndarray:
    if str(lateral_axis).strip().lower() == "x":
        return np.array([1.0, 0.0, 0.0], dtype=float)
    # Rotated rows retreat/approach from negative Y in tip-space.
...
class GCodeWriter:
    def __init__(
        ...
        self.b_uncurl_feed = float(DEFAULT_B_UNCURL_FEED)
        ...
        self.cur_motion_phase: str = canonical_motion_phase(self.cal.active_phase)
    ...
    def bc_for_tangent(
        ...
        if mode == "fixed":
            target_b, target_phase = choose_b_for_target_tip_angle(
                self.cal,
                0.0,
                prev_b=self.cur_b if self.cur_stage_xyz is not None else None,
                prev_phase=self.cur_motion_phase,
                search_samples=self.bc_solve_samples,
            )
            return float(target_b), float(c_target), target_phase
        target_b = desired_physical_b_angle_from_tangent(tangent)
        b, motion_phase = choose_b_for_target_tip_angle(
            self.cal,
            target_b,
            prev_b=self.cur_b if self.cur_stage_xyz is not None else None,
            prev_phase=self.cur_motion_phase,
        ...
        return float(b), float(c_target), motion_phase
    ...
    def tip_to_stage(
        ...
        if self.write_mode == "cartesian":
            ...
            b, c, motion_phase = (0.0, 0.0, self.cur_motion_phase) if tangent is None else self.bc_for_tangent(
                tangent,
                prev_c=prev_c,
        ...
        return self.clamp_stage(p_stage, "tip_to_stage"), float(b), float(c), motion_phase
    ...
    def write_move(self, p_stage: np.ndarray, b: float, c: float, feed: float, comment: Optional[str] = None, u_value: Optional[float] = None) -> None:
        ...
        next_phase = resolve_motion_phase(
            target_b=float(b),
            prev_b=self.cur_b if self.cur_stage_xyz is not None else None,
            prev_phase=self.cur_motion_phase,
            default_phase=self.cal.active_phase,
        )
        ...
        self.cur_b = float(b)
        self.cur_c = float(c)
        self.cur_motion_phase = next_phase
    ...
    def pre_rotate_c_at_current_pose(self, target_c: float, comment: Optional[str] = None) -> None:
        if self.write_mode != "calibrated" or self.cur_stage_xyz is None:
            return
        if abs(float(self.cur_c) - float(target_c)) <= 1e-9:
            return
        self.write_move(
    ...
    def pre_orient_bc_at_current_pose(
        self,
        target_b: float,
        target_c: float,
        comment: Optional[str] = None,
        ...
        if self.write_mode != "calibrated" or self.cur_stage_xyz is None:
            return
        if abs(float(self.cur_b) - float(target_b)) <= 1e-9 and abs(float(self.cur_c) - float(target_c)) <= 1e-9:
            return
        self.write_move(
            np.asarray(self.cur_stage_xyz, dtype=float),
            float(target_b),
            float(target_c),
            self.c_feed if feed is None else float(feed),
            comment=comment,
        )
    ...
    def approach_start_from_side(
        ...
        if str(kind).strip().lower() == "vertical" and int(row_num) in {3, 4}:
            ...
            overhead_stage[2] = max(current_stage_z, float(start_stage[2]) + float(side_lift_z))
            coarse_stage = np.asarray(start_stage, dtype=float).copy()
            fine_z_window = max(0.5, float(near_clearance))
            coarse_stage[2] = min(float(overhead_stage[2]), float(start_stage[2]) + fine_z_window)
            bc_release_feed = self.b_uncurl_feed if abs(float(self.cur_b) - float(start_b)) > 1e-9 else self.c_feed
            self.pre_orient_bc_at_current_pose(
                start_b,
                start_c,
                comment=f"{label}: pre-orient B/C before XY reposition",
                feed=bc_release_feed,
            )
            self.write_move(overhead_stage, start_b, start_c, self.travel_feed, comment=f"{label}: move in XY above vertical start")
            self.write_move(coarse_stage, start_b, start_c, self.travel_feed, comment=f"{label}: bulk straight Z descent above vertical start")
            self.write_move(np.asarray(start_stage, dtype=float), start_b, start_c, self.fine_approach_feed, comment=f"{label}: final straight Z descent to vertical start")
            self.cur_tip_xyz = np.asarray(start_tip, dtype=float).copy()
    ...
    def print_feed_for_path(self, label: str, row_num: int) -> float:
        if int(row_num) == 4 and label in {
            "plane3_xz_last3_fig5_curve_then_vertical_branch",
            "plane3_xz_last3_fig6_arc_then_slanted_branch",
        }:
            return float(DEFAULT_ROW4_CURVED_BRANCH_PRINT_FEED)
        return float(self.print_feed)
    ...
    def print_polyline(
        ...
        active_print_feed = self.print_feed_for_path(label=label, row_num=row_num)
        ...
        if abs(float(active_print_feed) - float(self.print_feed)) > 1e-9:
            self.f.write(f"; print feed override = {float(active_print_feed):.1f}\n")
...
def write_node_gcode(
    ...
    **_: Any,
    ...
    if write_mode == "calibrated":
        ...
        if bool(pull_phase_only) and cal.phase_models and "pull" in cal.phase_models:
            cal.phase_models["release"] = dict(cal.phase_models["pull"])
            cal.active_phase = "pull"
    else:
        cal = Calibration(
            pr=np.zeros(1),
            pz=np.zeros(1),
