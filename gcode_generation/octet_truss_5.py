...
DEFAULT_OUT = "gcode_generation/octet_truss_5.gcode"
...
# Geometry
DEFAULT_ORDER = 3                 # number of octet unit cells along cube edge
DEFAULT_PITCH_MM = 8.0           # octet strut length (FCC nearest-neighbor distance)
...
# Extrusion
DEFAULT_EXTRUSION_PER_MM = 0.0015    # U mm per mm of printed path; set >0 to enable U extrusion
...
def transverse_radius_from_b(cal: Calibration, b: float, phase: str = 'pull') -> float:
    r, _, y_off = predict_r_z_offplane(cal, b, phase=phase)
    return float(math.hypot(r, y_off))


def transverse_phase_from_b(cal: Calibration, b: float, phase: str = 'pull') -> float:
    return float(math.atan2(predict_r_z_offplane(cal, b, phase=phase)[2], predict_r_z_offplane(cal, b, phase=phase)[0]))
...
def solve_c_from_b_and_xy(cal: Calibration, b: float, x: float, y: float, phase: str = 'pull') -> float:
    phi = math.atan2(float(y), float(x))
    alpha = transverse_phase_from_b(cal, b, phase=phase)
...
def eval_tip_angle_pitch_from_vertical_deg(
    ...
    raw = curve_model_eval(_phase_curve_model(cal, 'tip_angle', phase), b)
...
def nozzle_axis_unit_xyz_from_vertical_pitch(
    ...
    a_deg = float(eval_tip_angle_pitch_from_vertical_deg(cal, b, angle_sign=angle_sign, angle_offset_deg=angle_offset_deg, phase=phase))
...
def sampled_ranges(
    ...
    rr = eval_r(cal, bb, phase=phase)
    yy = eval_offplane_y(cal, bb, phase=phase)
    zz = eval_z(cal, bb, phase=phase)
    aa_raw = curve_model_eval(_phase_curve_model(cal, 'tip_angle', phase), bb)
    aa_eff = eval_tip_angle_pitch_from_vertical_deg(cal, bb, angle_sign=angle_sign, angle_offset_deg=angle_offset_deg, phase=phase)
    return {
...
def find_b_for_pitch_target(
    ...
    aa = eval_tip_angle_pitch_from_vertical_deg(
        cal, bb, angle_sign=float(angle_sign), angle_offset_deg=float(angle_offset_deg), phase=phase
    )
...
def write_gcode_octet_truss(
    ...
    released_b_cmd = float(cal.b_max)
    ...
    def emit_tip_tracked_segmented_move(
        ...
        for j in range(1, len(tvals)):
            ...
            stage_xyz = p_tip - tip_offset_xyz_physical(cal, b_cmd, c_cmd, phase=phase)
    ...
    def emit_tip_fixed_reorient_via_release(
        fobj,
        ...
        if abs(cur_b - released_b_cmd) > 1e-9:
            current_pitch = float(
                eval_tip_angle_pitch_from_vertical_deg(
                    cal,
                    np.array([cur_b], dtype=float),
                    angle_sign=float(angle_sign),
                    angle_offset_deg=float(angle_offset_deg),
                    phase='pull',
                )[0]
            )
            release_b_start, release_pitch_eff = find_b_for_pitch_target(
                cal,
                b_lo=float(cal.b_min),
                b_hi=float(cal.b_max),
                target_pitch_deg=float(current_pitch),
                angle_sign=float(angle_sign),
                angle_offset_deg=float(angle_offset_deg),
                samples=max(4001, 4 * int(travel_samples) + 1),
                phase='release',
            )
            fobj.write(
                f"; EQUATION_SWITCH PCHIP_PULL_TO_PCHIP_RELEASE ({comment_prefix}) current_pitch={current_pitch:+.3f}deg matched_release_pitch={release_pitch_eff:+.3f}deg\n"
            )
            if abs(release_b_start - cur_b) > 1e-9:
                _, cur_b, cur_c = emit_tip_tracked_segmented_move(
                    fobj, tip_point, tip_point, cur_b, float(release_b_start), cur_c, cur_c,
                    samples=travel_samples,
                    feed=float(travel_feed),
                    comment=f"; {comment_prefix}: transfer from pull-state B to release-state B at matched tip angle",
                    context_prefix=f"{comment_prefix} transfer_to_release",
                    phase='release',
                )
            if abs(cur_b - released_b_cmd) > 1e-9:
                _, cur_b, cur_c = emit_tip_tracked_segmented_move(
                    fobj, tip_point, tip_point, cur_b, float(released_b_cmd), cur_c, cur_c,
                    samples=travel_samples,
                    feed=float(travel_feed),
                    comment=f"; {comment_prefix}: fully release B while holding tip fixed using release PCHIP",
                    context_prefix=f"{comment_prefix} release",
                    phase='release',
                )
        ...
        if abs(target_c - cur_c) > 1e-9:
            _, cur_b, cur_c = emit_tip_tracked_segmented_move(
                ...
                samples=travel_samples,
                feed=float(travel_feed),
                comment=f"; {comment_prefix}: rotate C at fully released B while holding tip fixed",
                context_prefix=f"{comment_prefix} rotate",
                phase='release',
            )

        if abs(float(b_to) - cur_b) > 1e-9:
            fobj.write(f"; EQUATION_SWITCH PCHIP_RELEASE_TO_PCHIP_PULL ({comment_prefix})\n")
            _, cur_b, cur_c = emit_tip_tracked_segmented_move(
                fobj, tip_point, tip_point, cur_b, float(b_to), cur_c, cur_c,
                samples=travel_samples,
                feed=float(travel_feed),
                comment=f"; {comment_prefix}: curl B from fully released state to target using pull PCHIP",
    ...
    def emit_raise_tip_to_safe_recenter_z(
        ...
        _, cur_b, cur_c = emit_tip_tracked_segmented_move(
            ...
            samples=travel_samples,
            feed=float(travel_feed),
            comment=f"; {comment_prefix}: raise tip above all written geometry before released-B C recenter",
    ...
    def emit_tip_fixed_c_recenter_if_needed(
        ...
        cur_b, cur_c = emit_tip_fixed_reorient_via_release(
            fobj,
            tip_point=safe_tip,
            b_from=float(cur_b),
            c_from=float(cur_c),
            b_to=float(b_restore),
            c_to=float(desired_c),
    ...
    with open(out_path, 'w') as f:
        ...
        f.write('; non-print travel policy: fully released-B travel for B changes, rotate C during XY travel, final Z descent last\n')
        f.write('; if XY travel would cross a previously written strut, lift above max written Z before XY travel\n')
        f.write('; same-tip turns are executed as: pull->release state match, fully release, C rotate, then release->pull and curl to target while holding tip fixed\n')
        f.write('; bounded C policy: choose equivalent C inside a configurable window and recenter only at safe high-Z fully released-B points\n')
            ...
            if first_edge:
                ...
                if same_tip_start:
                    ...
                    if same_tip_reorient_needed(float(current_b), float(current_c), float(b), float(target_c_same_tip)):
                        current_b, current_c = emit_tip_fixed_reorient_via_release(
                            f,
                            tip_point=p0_tip,
                        ...
                        f.write('; same-tip transition: orientation already aligned, skipping fully-released B reorientation\n')
                    current_tip = p0_tip.copy()
                else:
                    current_tip, current_b, current_c = emit_tip_fixed_c_recenter_if_needed(
                        f,
                    ...
                    current_b, current_c = emit_tip_fixed_reorient_via_release(
                        ...
                        b_from=float(current_b),
                        c_from=float(current_c),
                        b_to=float(released_b_cmd),
                        c_to=float(current_c),
                        comment_prefix='non-print travel release',
                    )
                    ...
                    if seg_idx + 1 < len(waypoints):
                        ...
                        if float(np.linalg.norm(bxy[:2] - a[:2])) > 1e-9:
                            ...
                            _, current_b, current_c = emit_tip_tracked_segmented_move(
                                ...
                                samples=travel_samples,
                                feed=float(travel_feed),
                                comment='; non-print travel: move in XY while rotating C at fully released B',
                    ...
                    if not has_xy_leg and abs(float(current_c) - float(c)) > 1e-9:
                        _, current_b, current_c = emit_tip_tracked_segmented_move(
                            ...
                            samples=travel_samples,
                            feed=float(travel_feed),
                            comment='; non-print travel: rotate C at fully released B while holding tip fixed',
                    ...
                    if abs(float(current_b) - float(b)) > 1e-9:
                        f.write('; EQUATION_SWITCH PCHIP_RELEASE_TO_PCHIP_PULL (non-print travel start curl)\n')
                        _, current_b, current_c = emit_tip_tracked_segmented_move(
                            f, p0_tip, p0_tip, float(current_b), float(b), float(current_c), float(current_c),
                            samples=travel_samples,
                            feed=float(travel_feed),
                            comment='; non-print travel: curl B from fully released state to print angle at start point using pull PCHIP',
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(
        description=(
            ...
            "for any B change the toolpath fully releases using the release PCHIP, rotates/travels while released, then curls with the pull PCHIP; XY travel remains collision-aware in tip space and can optionally lift above the written structure. "
            "Equivalent C commands are kept inside a bounded window and only recentred at safe high-Z fully released-B points. "
    ...
    ap.add_argument("--neutral-pitch-target-deg", type=float, default=DEFAULT_NEUTRAL_PITCH_TARGET_DEG,
                    help="Pitch-from-vertical target used only to report the legacy neutral B solve; actual B transitions now fully release to cal.b_max before re-curling with the pull PCHIP.")
    ...
    ap.add_argument("--c-recenter-margin-deg", type=float, default=DEFAULT_C_RECENTER_MARGIN_DEG,
                    help="When current C enters this margin near a C-window edge, recenter at a safe high-Z fully released-B point before travel or same-tip handling.")
