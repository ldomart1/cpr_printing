...
def choose_oriented_edge_plan(
    ...
    UPDATED behavior:
      - For non-horizontal edges (|dz| > horizontal_z_tol_mm), strongly prefer BOTTOM-UP
        direction (start lower Z -> end higher Z).
      - For horizontal edges, keep the existing head-front / travel / angle scoring.

    Preference order (after bottom-up diagonal rule):
    ...
    # --- NEW: force/prefer bottom-up on diagonals ---
    dz_ab = float(plan_ab.p1_tip[2] - plan_ab.p0_tip[2])  # >0 means bottom-up
    # dz_ba is approximately -dz_ab, but compute directly for robustness
    ...
    if not is_horizontal:
        ...
        # In a normal two-direction edge, exactly one should be bottom-up.
        # If so, choose it immediately (this is the requested behavior).
        if ab_bottom_up and not ba_bottom_up:
            return plan_ab, travel_ab
        if ba_bottom_up and not ab_bottom_up:
            return plan_ba, travel_ba
        # If both somehow tie (numerical weirdness), fall through to regular scoring.

    # --- Existing scoring for horizontals (and fallback ties) ---
...
def _continuous_horizontal_tip_pivot_transition(
    ...
    """
    True when next edge starts where previous edge ended (same tip vertex)
    and both edges are horizontal in tip space.
    """
    return (
...
def plan_all_edges_bottom_center_first(
    ...
    while remaining:
        # Prefer continuous horizontal chaining from current tip only when there are
        # no remaining diagonal struts below the current layer Z (bottom-up guard).
...
def _clamp_stage_xyz_to_bbox(
    ...
    """
    Clamp stage XYZ to a virtual bounding box, recording warnings when clamped.
    """
...
def write_gcode_tetrahedral_truss(
    ...
    with open(out_path, "w") as f:
        ...
        f.write(f"; virtual bbox (enforced during print/travel): "
                f"X[{float(virtual_bbox['x_min']):.3f}, {float(virtual_bbox['x_max']):.3f}] "
                f"Y[{float(virtual_bbox['y_min']):.3f}, {float(virtual_bbox['y_max']):.3f}] "
                f"Z[{float(virtual_bbox['z_min']):.3f}, {float(virtual_bbox['z_max']):.3f}]\n")
        f.write(f"; startup safe approach Z = {float(safe_approach_z):.3f}\n")
        f.write(f"; start pose (stage) = [{start_pose[0]:.3f}, {start_pose[1]:.3f}, {start_pose[2]:.3f}, "
                f"{start_pose[3]:.3f}, {start_pose[4]:.3f}]\n")
        f.write(f"; end pose (stage)   = [{end_pose[0]:.3f}, {end_pose[1]:.3f}, {end_pose[2]:.3f}, "
        ...
        # Safe startup move: go high in Z first, then XY, then dive to start Z.
        ...
        for ei, plan in enumerate(plans):
            ...
            b = plan.b_cmd
            c = plan.c_cmd
            chain_from_prev_horizontal = (
                ei > 0 and _continuous_horizontal_tip_pivot_transition(plans[ei - 1], plan)
            )
            ...
            if first_edge:
                ...
                first_edge = False
            elif chain_from_prev_horizontal:
                f.write("; continuous horizontal transition: tip-fixed pivot to next line direction\n")
                f.write("; (same tip vertex, new B/C and corresponding stage XYZ)\n")
                f.write(f"G1 {_fmt_axes_move(travel_axes)} F{float(travel_feed):.0f}\n")
            else:
            ...
            # End-of-pass dwell + optional pressure release before next transition
            next_is_continuous_horizontal = (
                (ei + 1) < len(plans)
                and _continuous_horizontal_tip_pivot_transition(plan, plans[ei + 1])
            )
            if emit_extrusion and float(pressure_offset_mm) > 0.0 and pressure_charged:
                ...
                if next_is_continuous_horizontal:
                    f.write("; keep pressure charged for continuous horizontal tip-pivot into next edge\n")
                else:
                    pressure_charged = False
        ...
        # User-defined end pose with same safe Z-first pattern.
