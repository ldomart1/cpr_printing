...
DEFAULT_RAW_IMAGE_EXTENSION = ".jpg"
DEFAULT_RAW_IMAGE_JPEG_QUALITY = 80
DEFAULT_ANALYSIS_CROP = {
    "crop_width_min": 1394,
    "crop_width_max": 2856,
    "crop_height_min": 767,
...
def save_compressed_image(
    ...
    if ext in (".jpg", ".jpeg"):
        quality = DEFAULT_RAW_IMAGE_JPEG_QUALITY if jpeg_quality is None else int(jpeg_quality)
...
class FixedTipPointTracker:
    ...
    def capture_and_save(
        ...
        if repeat_index is not None:
            extra += f"_REP{int(repeat_index):02d}"
        ...
        filename = (
            ...
            f"_B{b:.3f}_C{c:.3f}"
            f"{extra}"
            f"_{timestamp}{DEFAULT_RAW_IMAGE_EXTENSION}"
        ).replace(" ", "_")

        path = os.path.join(self.point_tracking_folder, filename)
        if ret and image is not None:
            save_compressed_image(output_path=path, image_bgr=image)
...
def _analysis_crop_state_path(project_dir: Path) -> Path:
    return project_dir / "analysis_crop.json"


def save_analysis_crop_state(project_dir: Path, analysis_crop: dict) -> Path:
    path = _analysis_crop_state_path(project_dir)
    with open(path, "w") as f:
        json.dump({"analysis_crop": _json_ready(analysis_crop)}, f, indent=2)
    print(f"[INFO] Saved analysis crop to: {path}")
    return path


def load_analysis_crop_state(project_dir: Path) -> Optional[dict]:
    path = _analysis_crop_state_path(project_dir)
    if not path.is_file():
        return None
    try:
        with open(path, "r") as f:
            payload = json.load(f)
        crop = payload.get("analysis_crop")
        if isinstance(crop, dict):
            return crop
    except Exception as exc:
        print(f"[WARN] Could not load saved analysis crop from {path}: {exc}")
    return None


def select_crop_before_acquisition(
    runner: "FixedTipPointTracker",
    default_crop: dict,
    project_dir: Path,
) -> dict:
    if runner.cam is None:
        raise RuntimeError("Camera must be connected before selecting crop.")

    ret, frame = runner.cam.read()
    if not ret or frame is None:
        ret, frame = runner.cam.read()
    if not ret or frame is None:
        raise RuntimeError("Could not capture live frame for pre-acquisition crop selection.")

    analysis_crop = interactive_crop_from_image(frame, default_crop=default_crop)
    save_analysis_crop_state(project_dir, analysis_crop)
    return analysis_crop
...
RE_REPEAT_IDX = re.compile(r"_REP(\d+)")
...
def parse_fixed_tip_filename_metadata(image_name: str) -> Dict[str, Any]:
    ...
    repeat_index = _safe_int_from_match(RE_REPEAT_IDX, stem)
...
def _apply_light_axes_style(ax, title: str, xlabel: str, ylabel: str):
    ax.set_title(title, color="#111827", fontsize=12.5, pad=10, weight="semibold")
    ax.set_xlabel(xlabel, color="#1f2937")
    ax.set_ylabel(ylabel, color="#1f2937")
    ax.tick_params(colors="#374151", labelsize=10)
    for spine in ax.spines.values():
        spine.set_color((0.12, 0.16, 0.22, 0.22))
        spine.set_linewidth(1.0)
    ax.grid(True, color=(0.12, 0.16, 0.22, 0.12), linewidth=0.8)
...
def _save_figure_formats(
    ...
    *,
    ...
    export_svg_pdf: bool = False,
) -> Dict[str, str]:
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    saved = {}
    png_path = out_dir / f"{stem}.png"
    fig.savefig(png_path, dpi=220, transparent=transparent, facecolor=facecolor)
    saved["png"] = str(png_path)
    if export_svg_pdf:
        svg_path = out_dir / f"{stem}.svg"
        pdf_path = out_dir / f"{stem}.pdf"
        fig.savefig(svg_path, transparent=transparent, facecolor=facecolor)
        fig.savefig(pdf_path, transparent=transparent, facecolor=facecolor)
        saved["svg"] = str(svg_path)
        saved["pdf"] = str(pdf_path)
    return saved


def _csv_cell_to_value(key: str, value: str) -> Any:
    if value == "":
        return None
    if key == "valid":
        return str(value).strip().lower() in ("1", "true", "yes", "y")
    if key in {"sample_index", "sample_index_from_name"}:
        try:
...
def load_tracked_tip_csv(csv_path: Path) -> List[Dict[str, Any]]:
    csv_path = Path(csv_path)
    rows: List[Dict[str, Any]] = []
    with open(csv_path, "r", newline="") as f:
        reader = csv.DictReader(f)
        for row in reader:
            parsed = {k: _csv_cell_to_value(k, v) for k, v in row.items()}
            image_name = parsed.get("image_name")
            if image_name:
                # Older tracked_tip_positions_mm.csv files do not carry the
                # filename-derived metadata columns. Recover them here so
                # processing-only reuse keeps repeat and phase grouping intact.
                meta = parse_fixed_tip_filename_metadata(str(image_name))
...
# =============================================================================
# Dead-band micro-release processing and plots
# =============================================================================

def _is_deadband_record(rec: Dict[str, Any]) -> bool:
    txt = " ".join(str(rec.get(k) or "") for k in ("phase_name", "motion_phase", "image_name")).lower()
    return ("deadband" in txt) or ("baseline" in txt)


def compute_deadband_micro_release_metrics(
    records: List[Dict[str, Any]],
    threshold_mm: float = 1.0,
) -> Dict[str, Any]:
    rows = [r for r in records if _is_deadband_record(r) and r.get("valid") and r.get("u_mm") is not None and r.get("z_mm") is not None]
    groups: Dict[Tuple[float, int], List[Dict[str, Any]]] = {}
    for rec in rows:
        a = _finite_float_or_none(rec.get("tip_angle_deg_from_name"))
        if a is None:
            continue
        repeat_index = rec.get("repeat_index")
        try:
            rep = int(repeat_index) if repeat_index is not None else 1
        except Exception:
            rep = 1
        groups.setdefault((float(round(a, 6)), rep), []).append(rec)

    out_rows: List[Dict[str, Any]] = []
    per_repeat_summary: List[Dict[str, Any]] = []
    thr = float(threshold_mm)
    for (attack, rep), group in sorted(groups.items(), key=lambda kv: (kv[0][0], kv[0][1])):
        group.sort(key=lambda r: int(r.get("sample_index", 0)))
        baseline_candidates = [r for r in group if str(r.get("phase_name") or "").lower().startswith("baseline")]
        baseline = baseline_candidates[0] if baseline_candidates else group[0]
        u0 = float(baseline["u_mm"])
        z0 = float(baseline["z_mm"])
        b0 = _finite_float_or_none(baseline.get("b_cmd"))
        release_rows = [r for r in group if str(r.get("motion_phase") or "").lower().startswith("release")]
        release_rows.sort(key=lambda r: int(r.get("sample_index", 0)))

        detected = None
        for idx, rec in enumerate(release_rows, start=1):
            u = float(rec["u_mm"])
            z = float(rec["z_mm"])
            disp = float(math.hypot(u - u0, z - z0))
            b = _finite_float_or_none(rec.get("b_cmd"))
            b_rel = None if b0 is None or b is None else float(b - b0)
            rec["deadband_attack_angle_deg"] = float(attack)
            rec["repeat_index"] = int(rep)
            rec["deadband_release_frame_index"] = int(idx)
            rec["deadband_displacement_from_baseline_mm"] = float(disp)
            rec["deadband_b_release_from_apogee_mm"] = b_rel
            rec["deadband_detected"] = bool(disp >= thr)
            out_rows.append(rec)
            if detected is None and disp >= thr:
                detected = {
                    "attack_angle_deg": float(attack),
                    "repeat_index": int(rep),
                    "frames_to_1mm": int(idx),
                    "release_steps_to_threshold": int(idx),
                    "b_release_to_threshold_mm": b_rel,
                    "b_cmd_at_threshold": b,
                    "displacement_at_threshold_mm": float(disp),
                    "baseline_sample_index": int(baseline.get("sample_index", 0)),
                    "threshold_sample_index": int(rec.get("sample_index", 0)),
                    "threshold_image_name": rec.get("image_name"),
                    "baseline_image_name": baseline.get("image_name"),
                }
        if detected is None:
            detected = {
                "attack_angle_deg": float(attack),
                "repeat_index": int(rep),
                "frames_to_1mm": None,
                "release_steps_to_threshold": None,
                "b_release_to_threshold_mm": None,
                "b_cmd_at_threshold": None,
                "displacement_at_threshold_mm": None,
                "baseline_sample_index": int(baseline.get("sample_index", 0)),
                "threshold_sample_index": None,
                "threshold_image_name": None,
                "baseline_image_name": baseline.get("image_name"),
                "not_reached": True,
            }
        detected["threshold_mm"] = float(thr)
        detected["release_frames_captured"] = int(len(release_rows))
        per_repeat_summary.append(detected)

    attack_summary: List[Dict[str, Any]] = []
    by_attack: Dict[float, List[Dict[str, Any]]] = {}
    for row in per_repeat_summary:
        by_attack.setdefault(float(row["attack_angle_deg"]), []).append(row)
    for attack, runs in sorted(by_attack.items(), key=lambda kv: kv[0]):
        vals_b = [float(r["b_release_to_threshold_mm"]) for r in runs if r.get("b_release_to_threshold_mm") is not None]
        vals_steps = [float(r["release_steps_to_threshold"]) for r in runs if r.get("release_steps_to_threshold") is not None]
        vals_frames = [float(r["frames_to_1mm"]) for r in runs if r.get("frames_to_1mm") is not None]
        attack_summary.append({
            "attack_angle_deg": float(attack),
            "n_runs": int(len(runs)),
            "n_detected": int(len(vals_b)),
            "b_release_to_threshold_mm_mean": (float(np.mean(vals_b)) if vals_b else None),
            "b_release_to_threshold_mm_std": (float(np.std(vals_b, ddof=1)) if len(vals_b) >= 2 else 0.0 if len(vals_b) == 1 else None),
            "release_steps_to_threshold_mean": (float(np.mean(vals_steps)) if vals_steps else None),
            "release_steps_to_threshold_std": (float(np.std(vals_steps, ddof=1)) if len(vals_steps) >= 2 else 0.0 if len(vals_steps) == 1 else None),
            "frames_to_1mm_mean": (float(np.mean(vals_frames)) if vals_frames else None),
            "frames_to_1mm_std": (float(np.std(vals_frames, ddof=1)) if len(vals_frames) >= 2 else 0.0 if len(vals_frames) == 1 else None),
        })

    return {
        "mode": "deadband_micro_release",
        "threshold_mm": float(thr),
        "num_groups": int(len(per_repeat_summary)),
        "per_repeat_summary": per_repeat_summary,
        "attack_summary": attack_summary,
        "annotated_records": out_rows,
        "summary": attack_summary,
    }


def save_deadband_micro_release_outputs(processed_dir: Path, records: List[Dict[str, Any]], threshold_mm: float = 1.0) -> Dict[str, Any]:
    processed_dir = Path(processed_dir)
    metrics = compute_deadband_micro_release_metrics(records, threshold_mm=threshold_mm)
    if not metrics.get("per_repeat_summary"):
        return metrics

    out_dir = processed_dir / "deadband_micro_release"
    light_dir = out_dir / "plots_light"
    dark_dir = out_dir / "plots_dark"
    out_dir.mkdir(parents=True, exist_ok=True)
    per_repeat_csv = out_dir / "deadband_per_repeat_summary.csv"
    attack_summary_csv = out_dir / "deadband_attack_angle_summary.csv"
    records_csv = out_dir / "deadband_release_per_frame.csv"
    summary_json = out_dir / "deadband_summary.json"

    per_repeat_fields = sorted({k for row in metrics["per_repeat_summary"] for k in row.keys()})
    with open(per_repeat_csv, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=per_repeat_fields)
        writer.writeheader(); writer.writerows(metrics["per_repeat_summary"])

    attack_fields = sorted({k for row in metrics["attack_summary"] for k in row.keys()})
    with open(attack_summary_csv, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=attack_fields)
        writer.writeheader(); writer.writerows(metrics["attack_summary"])

    record_fields = [
        "sample_index", "image_name", "repeat_index", "deadband_attack_angle_deg", "deadband_release_frame_index",
        "b_cmd", "deadband_b_release_from_apogee_mm", "u_mm", "z_mm",
        "deadband_displacement_from_baseline_mm", "deadband_detected",
    ]
    with open(records_csv, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=record_fields)
        writer.writeheader()
        for rec in metrics["annotated_records"]:
            writer.writerow({k: rec.get(k) for k in record_fields})

    with open(summary_json, "w") as f:
        json.dump(_json_ready(metrics), f, indent=2)

    per_repeat = metrics["per_repeat_summary"]
    attack_summary = metrics["attack_summary"]
    curve_blue = "#1f77b4"

    max_n_runs = max((int(s.get("n_runs") or 0) for s in attack_summary), default=0)
    microstep_size_mm = None
    finite_b_release = [
        float(s["b_release_to_threshold_mm"])
        for s in per_repeat
        if s.get("b_release_to_threshold_mm") is not None and s.get("release_steps_to_threshold") not in (None, 0)
    ]
    finite_steps = [
        float(s["release_steps_to_threshold"])
        for s in per_repeat
        if s.get("b_release_to_threshold_mm") is not None and s.get("release_steps_to_threshold") not in (None, 0)
    ]
    if finite_b_release and finite_steps:
        ratios = [b / st for b, st in zip(finite_b_release, finite_steps) if st != 0.0]
        if ratios:
            microstep_size_mm = float(np.median(np.asarray(ratios, dtype=float)))

    def _style_deadband_axes(ax, theme: str, title: str, xlabel: str, ylabel: str):
        if theme == "dark":
            _apply_dark_axes_style(ax, title, xlabel, ylabel)
        else:
            _apply_light_axes_style(ax, title, xlabel, ylabel)

    def _finalize_deadband_legend(ax, theme: str, ncol: int = 1, fontsize: Optional[float] = None):
        leg = ax.legend(fontsize=fontsize, ncol=ncol)
        if leg is None:
            return
        if theme == "dark":
            frame = leg.get_frame()
            frame.set_facecolor((0.06, 0.10, 0.15, 0.88))
            frame.set_edgecolor((0.75, 0.84, 0.93, 0.20))
            for txt in leg.get_texts():
                txt.set_color("#d7e2ee")

    def _plot_summary(theme: str) -> Dict[str, str]:
        ...
        if theme == "dark":
            ...
            fig.patch.set_alpha(1.0)
        run_x = np.asarray([float(s["attack_angle_deg"]) for s in per_repeat if s.get("b_release_to_threshold_mm") is not None], dtype=float)
        run_y = np.asarray([float(s["b_release_to_threshold_mm"]) for s in per_repeat if s.get("b_release_to_threshold_mm") is not None], dtype=float)
        if run_x.size:
            ax.scatter(
                run_x,
                run_y,
                s=44,
                alpha=(0.60 if theme == "dark" else 0.55),
                color=curve_blue,
                edgecolors="none",
                label="Runs",
            )
        mean_x = np.asarray([float(s["attack_angle_deg"]) for s in attack_summary if s.get("b_release_to_threshold_mm_mean") is not None], dtype=float)
        mean_y = np.asarray([float(s["b_release_to_threshold_mm_mean"]) for s in attack_summary if s.get("b_release_to_threshold_mm_mean") is not None], dtype=float)
        mean_err = np.asarray([0.0 if s.get("b_release_to_threshold_mm_std") is None else float(s.get("b_release_to_threshold_mm_std")) for s in attack_summary if s.get("b_release_to_threshold_mm_mean") is not None], dtype=float)
        if mean_x.size:
            ax.errorbar(
                mean_x,
                mean_y,
                ...
                capsize=4,
                color=curve_blue,
                label=f"Repeat mean (N={max_n_runs})",
            )
        _style_deadband_axes(
            ax,
            theme,
            "Dead-band vs apogee curl angle",
            "Apogee curl angle (deg)",
            "B-axis dead-band to 1 mm tip motion (mm)",
        )
        _finalize_deadband_legend(ax, theme)
        fig.tight_layout()
        saved = _save_figure_formats(
            fig,
            dark_dir if theme == "dark" else light_dir,
            "deadband_b_release_vs_attack_angle_runs_and_mean",
            transparent=(theme == "dark"),
            facecolor=("#0f1723" if theme == "dark" else "#ffffff"),
            export_svg_pdf=(theme == "light"),
        )
        plt.close(fig)
        return saved

    def _plot_steps(theme: str) -> Dict[str, str]:
        fig, ax = plt.subplots(figsize=(8.8, 4.8))
        if theme == "dark":
            fig.patch.set_alpha(0.0)
        else:
            fig.patch.set_facecolor("#ffffff")
            fig.patch.set_alpha(1.0)
        run_x = np.asarray([float(s["attack_angle_deg"]) for s in per_repeat if s.get("release_steps_to_threshold") is not None], dtype=float)
        run_y = np.asarray([float(s["release_steps_to_threshold"]) for s in per_repeat if s.get("release_steps_to_threshold") is not None], dtype=float)
        if microstep_size_mm is not None:
            run_y = run_y * float(microstep_size_mm)
        if run_x.size:
            ax.scatter(
                run_x,
                run_y,
                s=38,
                alpha=(0.55 if theme == "dark" else 0.50),
                color=curve_blue,
                edgecolors="none",
                label="Runs",
            )
        mean_x = np.asarray([float(s["attack_angle_deg"]) for s in attack_summary if s.get("release_steps_to_threshold_mean") is not None], dtype=float)
        mean_y = np.asarray([float(s["release_steps_to_threshold_mean"]) for s in attack_summary if s.get("release_steps_to_threshold_mean") is not None], dtype=float)
        mean_err = np.asarray([0.0 if s.get("release_steps_to_threshold_std") is None else float(s.get("release_steps_to_threshold_std")) for s in attack_summary if s.get("release_steps_to_threshold_mean") is not None], dtype=float)
        if microstep_size_mm is not None:
            mean_y = mean_y * float(microstep_size_mm)
            mean_err = mean_err * float(microstep_size_mm)
        if mean_x.size:
            ax.errorbar(
                mean_x,
                mean_y,
        ...
        _style_deadband_axes(
            ax,
            theme,
            "Tendon dead-band vs apogee curl angle",
            "Apogee curl angle (deg)",
            "Tendon displacement (mm) before tip motion threshold (1mm)",
        )
        _finalize_deadband_legend(ax, theme)
        fig.tight_layout()
        saved = _save_figure_formats(
            fig,
            dark_dir if theme == "dark" else light_dir,
            "deadband_release_steps_vs_attack_angle_runs_and_mean",
            transparent=(theme == "dark"),
            facecolor=("#0f1723" if theme == "dark" else "#ffffff"),
            export_svg_pdf=(theme == "light"),
        )
    ...
    def _plot_curves(theme: str) -> Dict[str, str]:
        ...
        if theme == "dark":
            ...
            fig.patch.set_facecolor("#ffffff")
            fig.patch.set_alpha(1.0)
        by_group: Dict[Tuple[float, int], List[Dict[str, Any]]] = {}
        for rec in metrics["annotated_records"]:
            a = float(rec["deadband_attack_angle_deg"])
            rep = int(rec.get("repeat_index") or 1)
            by_group.setdefault((a, rep), []).append(rec)
        for (a, rep), rows in sorted(by_group.items(), key=lambda kv: (kv[0][0], kv[0][1])):
            rows.sort(key=lambda r: int(r.get("deadband_release_frame_index", 0)))
            x = [float(r.get("deadband_b_release_from_apogee_mm")) for r in rows if r.get("deadband_b_release_from_apogee_mm") is not None]
            y = [float(r["deadband_displacement_from_baseline_mm"]) for r in rows if r.get("deadband_b_release_from_apogee_mm") is not None]
            if x:
                ax.plot(
                    x,
                    y,
                    linewidth=1.0,
                    alpha=(0.62 if theme == "dark" else 0.70),
                    color=curve_blue,
                    label=f"{a:g}° r{rep}",
                )
        ax.axhline(
            ...
            linestyle="--",
            linewidth=1.2,
            color=("#f3d67a" if theme == "dark" else "#b45309"),
            label=f"{threshold_mm:g} mm threshold",
        )
        _style_deadband_axes(
            ax,
            theme,
            "Dead-band release displacement curves",
            "B release from apogee (mm)",
            "Tip displacement from apogee baseline (mm)",
        )
        if len(by_group) <= 18:
            _finalize_deadband_legend(ax, theme, ncol=2, fontsize=7)
        fig.tight_layout()
        saved = _save_figure_formats(
            fig,
            dark_dir if theme == "dark" else light_dir,
            "deadband_release_displacement_curves",
            transparent=(theme == "dark"),
            facecolor=("#0f1723" if theme == "dark" else "#ffffff"),
            export_svg_pdf=(theme == "light"),
        )
    ...
    summary_plot_paths = _plot_summary("light")
    summary_plot_paths_dark = _plot_summary("dark")
    steps_plot_paths = _plot_steps("light")
    steps_plot_paths_dark = _plot_steps("dark")
    curves_plot_paths = _plot_curves("light")
    curves_plot_paths_dark = _plot_curves("dark")

    annotate_deadband_analysis_output_images(processed_dir / "analysis_outputs", metrics)

    metrics.update({
        "output_dir": str(out_dir),
        "plots_light_dir": str(light_dir),
        "plots_dark_dir": str(dark_dir),
        "per_repeat_csv": str(per_repeat_csv),
        "attack_summary_csv": str(attack_summary_csv),
        "records_csv": str(records_csv),
        "summary_json": str(summary_json),
        "summary_plot": summary_plot_paths.get("png"),
        "steps_plot": steps_plot_paths.get("png"),
        "curves_plot": curves_plot_paths.get("png"),
        "summary_plot_dark": summary_plot_paths_dark.get("png"),
        "steps_plot_dark": steps_plot_paths_dark.get("png"),
        "curves_plot_dark": curves_plot_paths_dark.get("png"),
        "plot_exports": {
            "light": {
                "summary": summary_plot_paths,
                "steps": steps_plot_paths,
                "curves": curves_plot_paths,
            },
            "dark": {
                "summary": summary_plot_paths_dark,
                "steps": steps_plot_paths_dark,
                "curves": curves_plot_paths_dark,
            },
        },
    })
    return metrics
...
def annotate_deadband_analysis_output_images(analysis_output_dir: Path, metrics: Dict[str, Any]):
    analysis_output_dir = Path(analysis_output_dir)
    if not analysis_output_dir.is_dir():
        return
    threshold = float(metrics.get("threshold_mm", 1.0))
    summary_by_key = {
        (float(s["attack_angle_deg"]), int(s.get("repeat_index") or 1)): s
        for s in metrics.get("per_repeat_summary", [])
    }
    for rec in metrics.get("annotated_records", []):
        image_name = rec.get("image_name")
        if not image_name:
            continue
        path = analysis_output_dir / f"{Path(image_name).stem}_analysis.png"
        if not path.is_file():
            continue
        img = cv2.imread(str(path), cv2.IMREAD_COLOR)
        if img is None:
            continue
        a = float(rec.get("deadband_attack_angle_deg"))
        rep = int(rec.get("repeat_index") or 1)
        disp = float(rec.get("deadband_displacement_from_baseline_mm", 0.0))
        frame_idx = int(rec.get("deadband_release_frame_index", 0))
        s = summary_by_key.get((a, rep), {})
        frames_to = s.get("frames_to_1mm")
        bthr = s.get("b_release_to_threshold_mm")
        status = "DETECTED" if bool(rec.get("deadband_detected")) else "below threshold"
        h, w = img.shape[:2]
        banner_h = max(76, int(round(0.11 * h)))
        banner = np.full((banner_h, w, 3), 245, dtype=np.uint8)
        line1 = f"Dead-band | apogee {a:.1f} deg | repeat {rep} | frame {frame_idx} | disp {disp:.3f} mm ({status})"
        line2 = (
            f"Threshold {threshold:.3f} mm | frames-to-threshold: {frames_to if frames_to is not None else 'not reached'}"
            f" | B@threshold: {bthr:.3f} mm" if bthr is not None else
            f"Threshold {threshold:.3f} mm | frames-to-threshold: {frames_to if frames_to is not None else 'not reached'}"
        )
        cv2.putText(banner, line1, (18, int(round(banner_h * 0.42))), cv2.FONT_HERSHEY_SIMPLEX, 0.68, (20, 20, 20), 2, lineType=cv2.LINE_AA)
        cv2.putText(banner, line2, (18, int(round(banner_h * 0.78))), cv2.FONT_HERSHEY_SIMPLEX, 0.58, (45, 45, 45), 2, lineType=cv2.LINE_AA)
        cv2.imwrite(str(path), np.vstack([banner, img]))
...
def run_processing(args=None):
    ...
    ap.add_argument("--reuse_tracked_tip_csv", "--reuse-tracked-tip-csv", dest="reuse_tracked_tip_csv", action="store_true",
                    help="Reuse processed_image_data_folder/tracked_tip_positions_mm.csv and skip image analysis.")
    ...
    tracked_csv_path = processed_dir / "tracked_tip_positions_mm.csv"
    reuse_tracked_tip_csv = bool(getattr(args, "reuse_tracked_tip_csv", False))

    if reuse_tracked_tip_csv:
        if not tracked_csv_path.is_file():
            raise FileNotFoundError(f"Requested --reuse-tracked-tip-csv but file does not exist: {tracked_csv_path}")
        print(f"[INFO] Reusing existing tracked-tip CSV: {tracked_csv_path}")
        records = load_tracked_tip_csv(tracked_csv_path)
        tracked_source = "tracked_tip_positions_mm.csv"
    else:
        cal = CTR_Shadow_Calibration(
        ...
        board_result = cal.estimate_board_reference_from_image(
            ...
            **_board_reference_kwargs(cal, args),
        )
        board_reference_debug_image = board_result.get("debug_image")
        _ = board_reference_debug_image
        ...
        analysis_crop = getattr(args, "preselected_analysis_crop", None)
        if not isinstance(analysis_crop, dict):
            analysis_crop = load_analysis_crop_state(project_dir)
        if isinstance(analysis_crop, dict):
            print("[INFO] Reusing saved analysis crop from pre-acquisition selection.")
        else:
    ...
    if str(getattr(args, "trajectory_mode", "")).strip().lower() == "deadband_micro_release":
        print("[INFO] Dead-band processing mode: generating deadband-only summaries and themed plot exports.")
        deadband_summary = save_deadband_micro_release_outputs(
            processed_dir=processed_dir,
            records=records,
            threshold_mm=float(getattr(args, "deadband_motion_threshold_mm", 1.0)),
        )
        if deadband_summary and deadband_summary.get("summary"):
            print(f"[INFO] Saved dead-band per-repeat CSV: {deadband_summary.get('per_repeat_csv')}")
            print(f"[INFO] Saved dead-band attack summary CSV: {deadband_summary.get('attack_summary_csv')}")
            print(f"[INFO] Saved dead-band per-frame CSV: {deadband_summary.get('records_csv')}")
            print(f"[INFO] Saved dead-band summary JSON: {deadband_summary.get('summary_json')}")
            print(f"[INFO] Saved light-theme plots: {deadband_summary.get('plots_light_dir')}")
            print(f"[INFO] Saved dark-theme plots: {deadband_summary.get('plots_dark_dir')}")
        return
    ...
    if deadband_summary and deadband_summary.get("summary"):
        ...
        print(f"[INFO] Saved dead-band per-frame CSV: {deadband_summary.get('records_csv')}")
        print(f"[INFO] Saved dead-band summary JSON: {deadband_summary.get('summary_json')}")
        print(f"[INFO] Saved dead-band light-theme plots: {deadband_summary.get('plots_light_dir')}")
        print(f"[INFO] Saved dead-band dark-theme plots: {deadband_summary.get('plots_dark_dir')}")
...
def run_acquisition_standard(args):
    cal = load_calibration(args.calibration, y_offset_fit=args.y_offset_fit)
    ...
    try:
        ...
        if bool(getattr(args, "select_crop_before_acquisition", False)):
            args.preselected_analysis_crop = select_crop_before_acquisition(
                runner=runner,
                default_crop=DEFAULT_ANALYSIS_CROP,
                project_dir=Path(runner.run_folder),
            )
        runner.connect_to_robot(args.duet_web_address)
        results = runner.execute_motion_and_capture(
            cal=cal,
...
# =============================================================================
# Dead-band micro-release acquisition mode
# =============================================================================

def _parse_deadband_roi(text: Optional[str]) -> Optional[Tuple[int, int, int, int]]:
    if text is None or str(text).strip() == "":
        return None
    parts = [p.strip() for p in str(text).replace(";", ",").split(",")]
    if len(parts) != 4:
        raise ValueError("--deadband-live-roi must be x,y,w,h")
    x, y, w, h = [int(round(float(p))) for p in parts]
    if w <= 0 or h <= 0:
        raise ValueError("--deadband-live-roi width/height must be positive")
    return x, y, w, h


def _deadband_live_tip_xy_from_image(
    image_path: str,
    roi: Optional[Tuple[int, int, int, int]] = None,
    mode: str = "auto",
) -> Optional[Tuple[float, float]]:
    """Simple online detector for dead-band stopping.

    mode:
      - "red_dot" / "auto_red_dot": prefer a red marker centroid.
      - "classical": use dark-foreground centroid.
      - "auto": try red first, then classical.

    This detector is only for optional online stopping; publishable displacement
    still comes from checkerboard-referenced processing of the saved images.
    """
    ...
    if roi is not None:
        x, y, w, h = roi
        x = max(0, min(int(x), img_bgr.shape[1] - 1))
        y = max(0, min(int(y), img_bgr.shape[0] - 1))
        w = max(1, min(int(w), img_bgr.shape[1] - x))
        h = max(1, min(int(h), img_bgr.shape[0] - y))
        view = img_bgr[y:y+h, x:x+w]
        x0, y0 = x, y

    def _centroid_from_mask(mask: np.ndarray, min_area: float = 8.0) -> Optional[Tuple[float, float]]:
        ...
        if abs(float(m.get("m00", 0.0))) < 1e-9:
            return None
        return float(m["m10"] / m["m00"]) + float(x0), float(m["m01"] / m["m00"]) + float(y0)

    mode_norm = str(mode or "auto").strip().lower()

    def _try_red() -> Optional[Tuple[float, float]]:
        ...
        return _centroid_from_mask(mask, min_area=6.0)

    def _try_classical() -> Optional[Tuple[float, float]]:
        gray = cv2.cvtColor(view, cv2.COLOR_BGR2GRAY)
        blur = cv2.GaussianBlur(gray, (5, 5), 0)
        _, bw = cv2.threshold(blur, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
        return _centroid_from_mask(bw, min_area=8.0)

    if mode_norm in ("red", "red_dot", "auto_red", "auto_red_dot", "red_marker"):
        pt = _try_red()
        if pt is not None:
            return pt
        if mode_norm.startswith("auto"):
            return _try_classical()
        return None

    if mode_norm == "classical":
        return _try_classical()
...
def _deadband_attack_values(min_deg: float, max_deg: float, step_deg: float) -> List[float]:
    lo = float(min_deg); hi = float(max_deg); step = abs(float(step_deg))
    if step <= 0:
        raise ValueError("--deadband-attack-step-deg must be positive")
    if lo > hi:
        lo, hi = hi, lo
    vals = []
    a = lo
    # Include the upper endpoint even with floating roundoff.
    while a <= hi + 1e-9:
        vals.append(float(round(a, 10)))
        a += step
    if not vals or abs(vals[-1] - hi) > 1e-6:
        vals.append(float(hi))
    return vals


def run_deadband_micro_release_acquisition(args):
    cal = load_calibration(args.calibration, y_offset_fit=args.y_offset_fit)
    attack_min = float(args.attack_min_deg)
    attack_max = float(args.attack_max_deg)
    active_fit_branch = resolve_fit_branch_for_window(
    ...
    cal.active_fit_branch = active_fit_branch
    print(f"[INFO] Active calibration fit branch for dead-band acquisition: {_fit_branch_title(active_fit_branch)}")
    ...
    base_stage_xyz = np.array([float(args.point_x), float(args.point_y), float(args.point_z)], dtype=float)
    c_deg = float(args.attack_c_deg)
    attack_values = _deadband_attack_values(attack_min, attack_max, float(args.deadband_attack_step_deg))
    n_repeats = max(1, int(getattr(args, "deadband_repeats", 5)))

    b_zero, used_zero, zero_meta = angle_to_b_for_fit(
        cal=cal,
    ...
    max_release_extension_mm = abs(float(args.deadband_max_release_extension_mm))
    max_steps = int(args.deadband_max_release_steps)
    extra_steps = max(0, int(args.deadband_extra_steps_after_detect))
    if max_release_extension_mm <= 0:
        raise ValueError("--deadband-max-release-extension-mm must be positive")
    if max_steps <= 0:
        raise ValueError("--deadband-max-release-steps must be positive")
    b_step = float(max_release_extension_mm) / float(max_steps)
    ...
    for lo, hi in [("x_min", "x_max"), ("y_min", "y_max"), ("z_min", "z_max")]:
        if virtual_bbox[lo] > virtual_bbox[hi]:
            virtual_bbox[lo], virtual_bbox[hi] = virtual_bbox[hi], virtual_bbox[lo]
    bbox_warnings: List[str] = []
    x_base, y_base, z_base = _clamp_stage_xyz_to_bbox(
        base_stage_xyz[0], base_stage_xyz[1], base_stage_xyz[2],
        virtual_bbox, "deadband base stage", bbox_warnings,
    )
    base_stage_xyz = np.array([x_base, y_base, z_base], dtype=float)
    ...
    meta = {
        "trajectory_mode": "deadband_micro_release",
        "description": "Curl to each apogee attack angle, then release B in fixed microsteps while holding XYZ/C fixed. Repeat each angle N times.",
        "fit_branch": _fit_branch_label(getattr(cal, "active_fit_branch", None)),
        "fit_branch_title": _fit_branch_title(getattr(cal, "active_fit_branch", None)),
        "angle_fit": str(args.angle_fit),
        "requested_attack_angle_range_deg": [float(args.attack_min_deg), float(args.attack_max_deg)],
        "used_attack_angle_range_deg": [float(attack_values[0]), float(attack_values[-1])],
        "deadband_attack_step_deg": float(args.deadband_attack_step_deg),
        "deadband_repeats": int(n_repeats),
        "deadband_b_release_step_mm": float(b_step),
        "deadband_max_release_extension_mm": float(max_release_extension_mm),
        "deadband_motion_threshold_mm": float(args.deadband_motion_threshold_mm),
        "deadband_extra_steps_after_detect": int(extra_steps),
        "deadband_max_release_steps": int(max_steps),
        "deadband_live_stop": bool(args.deadband_live_stop),
        "deadband_live_mm_per_px": float(args.deadband_live_mm_per_px),
        "deadband_live_threshold_px": float(args.deadband_live_threshold_px),
        "attack_values_deg": attack_values,
        "reset_attack_angle_deg": float(used_zero),
        "reset_b_cmd": float(b_zero),
        "stage_xyz_cmd": base_stage_xyz.astype(float).tolist(),
        "c_cmd_deg": float(c_deg),
        "feedrate_curl_and_release": 100.0,
        "model_meta_zero_release": zero_meta,
        "tip_detection_mode": str(getattr(args, "tip_detection_mode", "classical")),
    }
    ...
    try:
        ...
        print(f"[WARN] Could not save deadband metadata JSON: {e}")

    sample_counter = 0
    summary_rows: List[Dict[str, Any]] = []
    live_roi = _parse_deadband_roi(getattr(args, "deadband_live_roi", None))
    live_threshold_px = float(args.deadband_live_threshold_px)
    if bool(args.deadband_live_stop) and float(args.deadband_live_mm_per_px) > 0:
        live_threshold_px = float(args.deadband_motion_threshold_mm) / max(float(args.deadband_live_mm_per_px), 1e-12)
    live_tip_mode = str(getattr(args, "tip_detection_mode", "auto")).strip().lower()

    try:
        print(
            f"[INFO] Dead-band release plan | max extension={max_release_extension_mm:.6f} mm | "
            f"steps={max_steps} | microstep={b_step:.6f} mm"
        )
        runner.connect_to_camera(
            cam_port=int(args.cam_port),
            show_preview=bool(args.show_preview),
        ...
        runner.connect_to_robot(args.duet_web_address)
        runner._seed_commanded_pose(cal=cal, pose=start_pose)

        print("\nSafe startup approach...")
        runner._move_to_pose_safe(
            cal=cal,
            pose=start_pose,
            safe_approach_z=float(args.safe_approach_z),
            travel_feed=float(args.travel_feed),
            settle_s=float(args.travel_move_settle_s),
            use_estimated_waits=bool(args.settled_capture_mode),
            b_max_feed=float(args.b_max_feed),
        )

        print("\nMoving to fixed dead-band base pose at zero attack...")
        cmd = runner.send_absolute_move(
            100.0,
            **{
                cal.x_axis: float(base_stage_xyz[0]),
                cal.y_axis: float(base_stage_xyz[1]),
                cal.z_axis: float(base_stage_xyz[2]),
                cal.b_axis: float(b_zero),
                cal.c_axis: clamp_c_bounded(c_deg),
            }
        )
        if bool(args.settled_capture_mode):
            runner._record_estimated_motion(cal=cal, command_record=cmd, b_max_feed=float(args.b_max_feed))
            runner._wait_for_estimated_motion_complete(extra_settle=float(args.settled_capture_buffer_s), reason="deadband initial pose")
        else:
            runner.wait_for_duet_motion_complete(extra_settle=float(args.tracked_move_settle_s))

        for attack_idx, attack_deg in enumerate(attack_values):
            print("\n" + "-" * 72)
            print(f"Dead-band apogee {attack_idx + 1}/{len(attack_values)}: attack={attack_deg:.3f} deg | repeats={n_repeats}")
            print("-" * 72)
            b_apogee, used_attack, model_meta = angle_to_b_for_fit(
                cal=cal,
                requested_angle_deg=float(attack_deg),
                angle_fit=str(args.angle_fit),
                motion_phase="pull",
                inverse_samples=int(args.custom_inverse_samples),
            )
            block_name = infer_block_name_from_c(c_deg) or f"C{c_deg:g}"

            for repeat_idx in range(1, n_repeats + 1):
                print(f"\n  Repeat {repeat_idx}/{n_repeats}: curling to apogee requested={attack_deg:.3f} deg, used={used_attack:.3f} deg, B={b_apogee:.5f}")
                cmd = runner.send_absolute_move(
                    100.0,
                    **{
                        cal.x_axis: float(base_stage_xyz[0]),
                        cal.y_axis: float(base_stage_xyz[1]),
                        cal.z_axis: float(base_stage_xyz[2]),
                        cal.b_axis: float(b_apogee),
                        cal.c_axis: clamp_c_bounded(c_deg),
                    }
                )
                if bool(args.settled_capture_mode):
                    runner._record_estimated_motion(cal=cal, command_record=cmd, b_max_feed=float(args.b_max_feed))
                    runner._wait_for_estimated_motion_complete(extra_settle=float(args.settled_capture_buffer_s), reason="deadband curl apogee")
                else:
                    runner.wait_for_duet_motion_complete(extra_settle=float(args.tracked_move_settle_s))

                sample_counter += 1
                baseline_path = runner.capture_and_save(
                    sample_idx=sample_counter,
                    phase="baseline",
                    x=float(base_stage_xyz[0]), y=float(base_stage_xyz[1]), z=float(base_stage_xyz[2]),
                    b=float(b_apogee), c=clamp_c_bounded(c_deg),
                    flush_frames=int(args.camera_flush_frames),
                    tip_angle_deg=float(used_attack),
                    block_name=block_name,
                    block_phase_01=0.0,
                    motion_phase="pull",
                    repeat_index=int(repeat_idx),
                )
                baseline_xy = _deadband_live_tip_xy_from_image(baseline_path, roi=live_roi, mode=live_tip_mode) if baseline_path else None

                detected_step = None
                stop_after_step = max_steps
                step_idx = 0
                b_curr = float(b_apogee)
                for step_idx in range(1, max_steps + 1):
                    b_next = min(float(b_zero), float(b_apogee) + float(step_idx) * b_step)
                    cmd = runner.send_absolute_move(
                        100.0,
                        **{
                            cal.x_axis: float(base_stage_xyz[0]),
                            cal.y_axis: float(base_stage_xyz[1]),
                            cal.z_axis: float(base_stage_xyz[2]),
                            cal.b_axis: float(b_next),
                            cal.c_axis: clamp_c_bounded(c_deg),
                        }
                    )
                    if bool(args.settled_capture_mode):
                        runner._record_estimated_motion(cal=cal, command_record=cmd, b_max_feed=float(args.b_max_feed))
                        runner._wait_for_estimated_motion_complete(extra_settle=float(args.settled_capture_buffer_s), reason="deadband B release microstep")
                    else:
                        runner.wait_for_duet_motion_complete(extra_settle=float(args.tracked_move_settle_s))

                    sample_counter += 1
                    img_path = runner.capture_and_save(
                        sample_idx=sample_counter,
                        phase="deadband",
                        x=float(base_stage_xyz[0]), y=float(base_stage_xyz[1]), z=float(base_stage_xyz[2]),
                        b=float(b_next), c=clamp_c_bounded(c_deg),
                        flush_frames=int(args.camera_flush_frames),
                        tip_angle_deg=float(used_attack),
                        block_name=block_name,
                        block_phase_01=float(step_idx),
                        motion_phase="release",
                        repeat_index=int(repeat_idx),
                    )
                    live_disp_px = None
                    if bool(args.deadband_live_stop) and baseline_xy is not None and img_path:
                        curr_xy = _deadband_live_tip_xy_from_image(img_path, roi=live_roi, mode=live_tip_mode)
                        if curr_xy is not None:
                            live_disp_px = float(math.hypot(curr_xy[0] - baseline_xy[0], curr_xy[1] - baseline_xy[1]))
                            if detected_step is None and live_disp_px >= live_threshold_px:
                                detected_step = int(step_idx)
                                stop_after_step = min(max_steps, int(step_idx) + extra_steps)
                                print(
                                    f"[LIVE] motion detected at release step {detected_step} "
                                    f"({live_disp_px:.2f} px); capturing through step {stop_after_step}."
                                )
                    b_curr = b_next
                    if step_idx >= stop_after_step:
                        break
                    if math.isclose(b_next, float(b_zero), abs_tol=1e-9):
                        break

                summary_rows.append({
                    "apogee_index": int(attack_idx),
                    "repeat_index": int(repeat_idx),
                    "requested_attack_angle_deg": float(attack_deg),
                    "used_attack_angle_deg": float(used_attack),
                    "b_apogee_cmd": float(b_apogee),
                    "b_reset_cmd": float(b_zero),
                    "b_last_release_cmd": float(b_curr),
                    "b_release_step_mm": float(b_step),
                    "release_steps_captured": int(step_idx),
                    "live_detected_step": detected_step,
                    "live_threshold_px": float(live_threshold_px),
                    "model_label": model_meta.get("selected_model_label"),
                })

                print(f"  Returning to zero-attack release B={b_zero:.5f} before next repeat/angle...")
                cmd = runner.send_absolute_move(
                    100.0,
                    **{
                        cal.x_axis: float(base_stage_xyz[0]),
                ...
                if bool(args.settled_capture_mode):
                    runner._record_estimated_motion(cal=cal, command_record=cmd, b_max_feed=float(args.b_max_feed))
                    runner._wait_for_estimated_motion_complete(extra_settle=float(args.settled_capture_buffer_s), reason="deadband reset")
                else:
                    runner.wait_for_duet_motion_complete(extra_settle=float(args.tracked_move_settle_s))
                if float(args.deadband_reset_dwell_s) > 0:
                    time.sleep(float(args.deadband_reset_dwell_s))
        ...
        command_csv_path = runner.write_command_log_csv(cal)
        summary_csv = Path(runner.run_folder) / "deadband_acquisition_summary.csv"
        with open(summary_csv, "w", newline="") as f:
            fieldnames = sorted({k for row in summary_rows for k in row.keys()}) if summary_rows else []
            if fieldnames:
                writer = csv.DictWriter(f, fieldnames=fieldnames)
                writer.writeheader()
                writer.writerows(summary_rows)
        print("\nDead-band acquisition complete.")
        print(f"Images saved: {sample_counter}")
        print(f"Commanded motor positions CSV: {command_csv_path}")
        print(f"Dead-band acquisition summary CSV: {summary_csv}")
        if bbox_warnings:
            for msg in bbox_warnings:
                print(msg)
        args.project_dir = runner.run_folder
...
def run_acquisition(args):
    if str(getattr(args, "trajectory_mode", "")).strip().lower() == "deadband_micro_release":
        return run_deadband_micro_release_acquisition(args)
    return run_acquisition_standard(args)
...
def add_unified_cli_args(ap: argparse.ArgumentParser):
    ...
    ap.add_argument("--select-crop-before-acquisition", action="store_true",
                    help="Before any robot motion, grab a live camera frame, open the crop GUI, save the selected crop in the project folder, and reuse it during processing.")
    ...
    ap.add_argument("--trajectory-mode", type=str, default="angle_command",
                    ...
                    help=(
                        ...
                        "deadband_micro_release curls to apogee attack angles, then releases B in microsteps to characterize backlash/dead-band."
                    ))
    ...
    # Dead-band micro-release characterization
    ap.add_argument("--deadband-attack-step-deg", type=float, default=5.0,
                    help="Attack-angle apogee increment for deadband_micro_release. Default 5 deg.")
    ap.add_argument("--deadband-b-release-step-mm", type=float, default=0.01,
                    help="Deprecated for deadband_micro_release. The acquisition now computes the microstep as --deadband-max-release-extension-mm / --deadband-max-release-steps.")
    ap.add_argument("--deadband-max-release-extension-mm", type=float, default=1.0,
                    help="Total B release extension budget used to compute the acquisition microstep size. For example, 1.0 mm over 80 steps gives 0.0125 mm per microstep.")
    ap.add_argument("--deadband-motion-threshold-mm", type=float, default=1.0,
                    help="Offline threshold: displacement from the apogee baseline that counts as tip motion. Default 1 mm.")
    ap.add_argument("--deadband-extra-steps-after-detect", type=int, default=20,
                    help="After live detection, keep capturing this many extra release microsteps. Default 20.")
    ap.add_argument("--deadband-repeats", type=int, default=5,
                    help="Number of repeated dead-band runs to acquire at each apogee attack angle before moving to the next angle. Default 5.")
    ap.add_argument("--deadband-max-release-steps", type=int, default=400,
                    help="Safety cap for release microsteps per apogee if live detection is disabled or not reached. Default 400.")
    ap.add_argument("--deadband-live-stop", action="store_true",
                    help="Use a simple live image-space detector to stop each release after threshold + extra steps. Without this, acquisition captures up to --deadband-max-release-steps and offline processing finds the threshold.")
    ap.add_argument("--deadband-live-mm-per-px", type=float, default=0.0,
                    help="Approximate live camera scale for --deadband-live-stop. If <=0, --deadband-live-threshold-px is used directly.")
    ap.add_argument("--deadband-live-threshold-px", type=float, default=30.0,
                    help="Live image-space stop threshold in pixels when --deadband-live-mm-per-px <= 0. Default 30 px.")
    ap.add_argument("--deadband-live-roi", type=str, default=None,
                    help="Optional live detector ROI as x,y,w,h in pixels. Recommended for reliable live stopping.")
    ap.add_argument("--deadband-reset-dwell-s", type=float, default=1.0,
                    help="Dwell after returning to the initial zero-attack B position before the next apogee. Default 1 s.")
    ...
    ap.add_argument("--reuse_tracked_tip_csv", "--reuse-tracked-tip-csv", dest="reuse_tracked_tip_csv", action="store_true",
                    help="Reuse processed_image_data_folder/tracked_tip_positions_mm.csv and skip image analysis. Intended for processing-only deadband plot regeneration.")
...
def main():
    ...
    if bool(args.run_processing):
        ...
        if bool(getattr(args, "reuse_tracked_tip_csv", False)):
            if str(getattr(args, "trajectory_mode", "")).strip().lower() != "deadband_micro_release":
                ap.error("--reuse-tracked-tip-csv currently supports only --trajectory-mode deadband_micro_release.")
        else:
