...
DEFAULT_MACHINE_START_Y = 100.0
DEFAULT_MACHINE_START_Z = -10.0
DEFAULT_MACHINE_START_B = 0.0
DEFAULT_MACHINE_START_C = 0.0

DEFAULT_MACHINE_END_X = 100.0
DEFAULT_MACHINE_END_Y = 100.0
DEFAULT_MACHINE_END_Z = -10.0
DEFAULT_MACHINE_END_B = 0.0
DEFAULT_MACHINE_END_C = 0.0

DEFAULT_TRAVEL_FEED = 3000.0            # mm/min
...
DEFAULT_PREFLOW_DWELL_MS = 250
DEFAULT_START_NODE_DWELL_MS = 250
DEFAULT_NODE_DWELL_MS = 40
...
DEFAULT_END_OVEREXTENSION_DIRECTION_WINDOW_MM = 5.0
DEFAULT_END_OVEREXTENSION_FEED = 5000.0
...
DEFAULT_AORTA_CYLINDER_CLEARANCE_MM = 5.0
DEFAULT_AORTA_CYLINDER_MIN_RADIUS_MM = 8
DEFAULT_AORTA_CYLINDER_SAMPLES = 12
...
DEFAULT_RADIUS_FEED_MAP = "0.15:1000,0.20:800,0.25:700,0.30:650,0.40:400,0.60:200,1.00:80,1.30:70"

DEFAULT_WRITE_MODE = "calibrated"
DEFAULT_ORIENTATION_MODE = "fixed"
...
def _select_offplane_model(
    ...
    requested_mode = None if requested_offplane_fit_mode is None else str(requested_offplane_fit_mode).strip().lower()
    ...
    def pick(models: Dict[str, Any], names: Sequence[str]) -> Optional[Dict[str, Any]]:
        for key in names:
            spec = _normalize_model_spec(models.get(key))
            if spec is not None:
                return spec
    ...
    if requested_mode and requested_mode != "auto":
        explicit_specs: Dict[str, Tuple[Dict[str, Any], List[str], List[str]]] = {
            "pull_pchip": (
                phase_models,
        ...
        if payload is None:
            raise ValueError(
                f"Unsupported requested offplane fit mode: {requested_offplane_fit_mode}. "
...
def load_calibration(json_path: str, requested_offplane_fit_mode: Optional[str] = None) -> Calibration:
    p = Path(json_path)
    if not p.exists():
    ...
    y_off_model, y_off_extrap_model = _select_offplane_model(
        active_phase_models,
        fit_models if isinstance(fit_models, dict) else {},
        selected_offplane_fit_model,
        selected_fit_model,
        requested_offplane_fit_mode=requested_offplane_fit_mode,
    )
        ...
        for line_no, raw_line in enumerate(f, start=1):
            ...
            if len(parts) < 4:
                raise ValueError(
                    ...
                    "Expected: group_number dx dy dz [db_deg] [dc_deg] [feed_mm_min] [enabled] [node_dx_mm] [node_dy_mm] [node_dz_mm]"
                )
            ...
            node_dx = float(parts[8]) if len(parts) >= 9 else 0.0
            node_dy = float(parts[9]) if len(parts) >= 10 else 0.0
            node_dz = float(parts[10]) if len(parts) >= 11 else 0.0
            node_db = float(parts[11]) if len(parts) >= 12 else 0.0
            node_dc = float(parts[12]) if len(parts) >= 13 else 0.0

            out[group_number] = GroupDisplacement(
                group_number=group_number,
                delta_tip_xyz=np.array([dx, dy, dz], dtype=float),
                delta_b_deg=db,
                delta_c_deg=dc,
                node_gantry_offset_mm=np.array([node_dx, node_dy, node_dz], dtype=float),
                node_delta_b_deg=node_db,
                node_delta_c_deg=node_dc,
                feed=feed,
                enabled=enabled,
                note=note,
            )
...
def prepend_group_node_gantry_offset(
    points: np.ndarray,
    displacement: Optional[GroupDisplacement],
    ...
    node_gantry_offset = np.zeros(3, dtype=float)
    if cumulative_offset is not None:
        node_gantry_offset = node_gantry_offset + np.asarray(cumulative_offset, dtype=float).reshape(3)
    if node_gantry_offset.shape != (3,) or float(np.linalg.norm(node_gantry_offset)) <= 1e-12:
        return pts

    # Interactive tuning defines this as a machine/stage XYZ translation for the
    # whole branch, so headless save-gcode must apply the same semantics.
    pts_shifted = pts + node_gantry_offset[None, :]
    if float(np.linalg.norm(pts_shifted[0] - pts[0])) <= float(point_merge_tol):
        return pts
    return pts_shifted
...
class GCodeWriter:
    def __init__(
        ...
        end_overextension_mm: float = DEFAULT_END_OVEREXTENSION_MM,
        end_overextension_direction_window_mm: float = DEFAULT_END_OVEREXTENSION_DIRECTION_WINDOW_MM,
        ...
        self.end_overextension_mm = max(0.0, float(end_overextension_mm))
        self.end_overextension_direction_window_mm = max(0.0, float(end_overextension_direction_window_mm))
        self.end_overextension_feed = max(0.0, float(end_overextension_feed))
    ...
    def set_travel_acceleration(self, accel_mm_s2: float, comment: Optional[str] = None) -> None:
        ...
        if accel <= 0.0:
            return
        if self.active_travel_accel_mm_s2 is not None and abs(self.active_travel_accel_mm_s2 - accel) <= 1e-9:
            return
        if comment:
            self.f.write(f"; {comment}\n")
        self.f.write(f"M204 T{accel:.0f}\n")
        self.active_travel_accel_mm_s2 = accel

    def begin_post_print_travel(self, label: str) -> bool:
        if len(self.printed_paths) == 0:
            return False
        if self.travel_accel_mm_s2 <= 0.0:
            return False
        if self.post_print_travel_accel_scale >= 0.999999:
            return False
        reduced_accel = self.travel_accel_mm_s2 * self.post_print_travel_accel_scale
        self.set_travel_acceleration(
            reduced_accel,
            comment=(
                f"{label}: reduce travel acceleration after print to "
                f"{reduced_accel:.1f} mm/s^2"
            ),
        )
        return True

    def end_post_print_travel(self, label: str, active: bool) -> None:
        if not active:
            return
        self.set_travel_acceleration(
            self.travel_accel_mm_s2,
            comment=f"{label}: restore nominal travel acceleration",
        )
    ...
    def write_tip_tracking_pose_move(
        ...
        if self.write_mode != "calibrated":
            p_stage = self.clamp_stage(np.asarray(target_tip, dtype=float), "tip_tracking_pose_move_cartesian")
            self.write_move(p_stage, target_b, target_c, feed, comment=comment)
            self.cur_tip_xyz = np.asarray(target_tip, dtype=float).copy()
        ...
        tip_step = max(0.25, float(self.skeleton_collision_sample_step_mm) if max_tip_step_mm is None else float(max_tip_step_mm))
        b_step = max(0.25, float(self.b_max_step_deg) if max_b_step_deg is None else float(max_b_step_deg))
        c_step = max(0.25, float(self.c_max_step_deg) if max_c_step_deg is None else float(max_c_step_deg))
        n_steps = max(
            1,
        ...
        for step_idx in range(1, n_steps + 1):
            ...
            axes: List[Tuple[str, float]] = [
                ...
                (self.cal.z_axis, float(p_stage_i[2])),
                (self.cal.b_axis, float(b_i)),
                (self.cal.c_axis, float(c_i)),
            ]
            self.f.write(f"G1 {_fmt_axes_move(axes)} F{float(feed):.0f}\n")
            self.cur_stage_xyz = p_stage_i.copy()
    ...
    def apply_group_displacement(self, displacement: GroupDisplacement, default_feed: float, label: str) -> None:
        ...
        delta_stage = np.asarray(displacement.delta_tip_xyz, dtype=float)
        if (
        ...
        self.write_move(
            ...
            comment=(
                ...
                f"dStageXYZ=[{delta_stage[0]:.3f}, {delta_stage[1]:.3f}, {delta_stage[2]:.3f}] "
    ...
    def approach_start_from_aorta_cylinder(
        ...
        if (
            ...
            or self.orientation_mode == "fixed"
            or self.aorta_centerline_points is None
            or len(self.printed_paths) == 0
        ):
        ...
        try:
            ...
            self.write_tip_tracking_pose_move(
                target_tip=prespin_tip,
                target_b=start_b,
                target_c=start_c,
                feed=final_feed,
                comment=f"{label}: fixed-tip pre-spin at fine-approach point to print orientation",
    ...
    def approach_start_direct(
        ...
        try:
            ...
            dist = float(np.linalg.norm(to_start))
            if dist <= 1e-9:
                self.move_to_tip_with_explicit_bc(
                    start_tip,
                    b=start_b,
            ...
            # Reorient at the current tip before any Z change toward the node.
            if abs(float(cur_tip[2]) - float(start_tip[2])) > 1e-9:
                self.move_to_tip_with_explicit_bc(
                    cur_tip,
                    b=start_b,
                    c=start_c,
            ...
            fine_dist = min(float(self.fine_approach_distance), dist)
            if dist - fine_dist > 1e-9:
                fine_start_tip = cur_tip + ((dist - fine_dist) / dist) * to_start
                self.move_to_tip_with_explicit_bc(
                    fine_start_tip,
                    b=start_b,
                    c=start_c,
    ...
    def terminal_overextension_direction(self, points: np.ndarray, tangents: np.ndarray) -> np.ndarray:
        pts = np.asarray(points, dtype=float)
        tan = np.asarray(tangents, dtype=float)
        if len(pts) < 2:
            fallback = tan[-1] if len(tan) > 0 else np.array([0.0, 0.0, 1.0], dtype=float)
            return normalize(fallback)

        window = float(self.end_overextension_direction_window_mm)
        if window <= 1e-9:
            return normalize(tan[-1])

        end_pt = pts[-1].copy()
        remaining = window
        cursor = end_pt.copy()
        for idx in range(len(pts) - 1, 0, -1):
            p_prev = pts[idx - 1]
            p_cur = pts[idx]
            seg = p_cur - p_prev
            ...
            if seg_len >= remaining:
                start_pt = p_cur - (remaining / seg_len) * seg
                direction = normalize(end_pt - start_pt)
                return normalize(direction if np.linalg.norm(direction) > 1e-12 else tan[-1])
            remaining -= seg_len
            cursor = p_prev.copy()

        direction = normalize(end_pt - cursor)
    ...
    def print_polyline(
        ...
        if self.end_overextension_mm > 1e-9:
            extension_dir = self.terminal_overextension_direction(points, tangents)
            extension_tip = np.asarray(points[-1], dtype=float) + extension_dir * float(self.end_overextension_mm)
            self.move_to_tip(
                extension_tip,
                tangent=extension_dir,
                feed=self.end_overextension_feed,
                comment=(
                    f"{label}: pressure-off terminal overextension "
                    f"{float(self.end_overextension_mm):.3f} mm along terminal branch direction"
                ),
            )
...
def build_group_parent_map(paths: Sequence[PrintPath]) -> Dict[int, Optional[int]]:
    vessel_to_group: Dict[int, int] = {}
    for group_idx, path in enumerate(paths, start=1):
        for vessel_id in path.source_vessel_ids:
            vessel_to_group[int(vessel_id)] = int(group_idx)

    out: Dict[int, Optional[int]] = {}
    for group_idx, path in enumerate(paths, start=1):
        parent_group = None
        if path.parent_vessel_id is not None:
            parent_group = vessel_to_group.get(int(path.parent_vessel_id))
            if parent_group == group_idx:
                parent_group = None
        out[int(group_idx)] = parent_group
    return out


def cumulative_group_node_offset_mm(
    group_number: int,
    group_displacements: Optional[Dict[int, GroupDisplacement]],
    group_parent_map: Optional[Dict[int, Optional[int]]],
) -> np.ndarray:
    total = np.zeros(3, dtype=float)
    if group_displacements is None:
        return total
    seen: set[int] = set()
    current: Optional[int] = int(group_number)
    while current is not None and current not in seen:
        seen.add(current)
        disp = group_displacements.get(int(current))
        if disp is not None:
            total = np.asarray(disp.node_gantry_offset_mm, dtype=float).reshape(3) + total
        current = None if group_parent_map is None else group_parent_map.get(int(current))
    return total
...
def extend_branch_starts_into_printed_tree(
    ...
    # Retained for CLI/API compatibility; branch-start overlap now uses the
    # branch tangent directly instead of estimating an upstream parent tangent.
    _ = tangent_window_mm
    ...
    for path in paths:
        ...
        # Find the true connection point on the already printed tree.
        start = path.connection_xyz if path.connection_xyz is not None else pts[0]
        best_point = start.copy()
        best_dist = float("inf")
        for printed in printed_paths:
            q, _, _, dist = projected_point_and_arclength_on_polyline(start, printed)
            if dist < best_dist:
                best_point = q
                best_dist = dist

        # Use the branch tangent, not the parent tangent, so the branch start
        # overextends slightly before the attachment point along the branch.
        branch_tangent = normalize(pts[1] - pts[0])
        if float(np.linalg.norm(branch_tangent)) <= 1e-12:
            branch_tangent = normalize(pts[-1] - pts[0])
...
def write_vessel_gcode(
    ...
    enable_side_approach: bool = True,
    ...
    with open(out_path, "w") as f:
        ...
        f.write(f"; requested_offplane_fit_mode = {cal.requested_offplane_fit_mode or 'auto'}\n")
        f.write(f"; active_phase = {cal.active_phase}\n")
        f.write(f"; terminal_overextension_mm = {float(DEFAULT_END_OVEREXTENSION_MM):.6f}\n")
        f.write(f"; terminal_overextension_direction_window_mm = {float(DEFAULT_END_OVEREXTENSION_DIRECTION_WINDOW_MM):.6f}\n")
        f.write(f"; terminal_overextension_feed = {float(DEFAULT_END_OVEREXTENSION_FEED):.6f}\n")
        ...
        if float(travel_accel_mm_s2) > 0.0:
            g.set_travel_acceleration(
                float(travel_accel_mm_s2),
        ...
        for group_idx, path in enumerate(paths, start=1):
            displacement = None if group_displacements is None else group_displacements.get(group_idx)
            cumulative_offset = cumulative_group_node_offset_mm(group_idx, group_displacements, group_parent_map)
            points = prepend_group_node_gantry_offset(
                points=np.asarray(path.points, dtype=float),
                displacement=displacement,
                point_merge_tol=point_merge_tol,
                cumulative_offset=cumulative_offset,
            ...
            if displacement is not None and (
                ...
                g.move_to_tip_with_explicit_bc(
                    ...
                    comment=(
                        ...
                        f"dB={float(displacement.node_delta_b_deg):.3f} "
                        f"dC={float(displacement.node_delta_c_deg):.3f}"
                    ),
                )
            g.print_polyline(
                points,
        ...
        if g.cur_stage_xyz is None:
            raise RuntimeError("Cannot emit shutdown lift before any machine pose has been established.")
        shutdown_stage = np.asarray(g.cur_stage_xyz, dtype=float).copy()
        shutdown_stage[2] = shutdown_z
        g.write_move(
            shutdown_stage,
            g.cur_b,
            g.cur_c,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--use-explicit-machine-start-end", action="store_true", help="Use the explicit machine start pose and explicit shutdown Z instead of anchoring the start to the first written point.")
    ...
    ap.add_argument("--travel-accel-mm-s2", type=float, default=DEFAULT_TRAVEL_ACCEL_MM_S2, help="Nominal travel acceleration for non-print moves. 0 leaves the machine's current acceleration unchanged.")
...
def main(args: argparse.Namespace) -> None:
    ...
    if write_mode == "calibrated":
        ...
        cal = load_calibration(args.calibration, requested_offplane_fit_mode=str(args.offplane_fit_mode))
    else:
        cal = Calibration(
            pr=np.zeros(1),
            pz=np.zeros(1),
