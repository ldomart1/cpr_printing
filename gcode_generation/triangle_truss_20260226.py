...
def choose_oriented_edge_plan(
    ...
    endpoint_snap_tol_mm: float = 1e-6,
    ...
    UPDATED behavior:
      ...
      - For horizontal edges, if one endpoint is the current tip position, start there.
        Otherwise keep the existing head-front / travel / angle scoring.
    ...
    # For horizontals, prefer continuing from the current tip vertex when exact match exists.
    if is_horizontal and current_tip is not None:
        ab_at_current = travel_ab <= float(endpoint_snap_tol_mm)
        ba_at_current = travel_ba <= float(endpoint_snap_tol_mm)
        if ab_at_current and not ba_at_current:
            return plan_ab, travel_ab
        if ba_at_current and not ab_at_current:
            return plan_ba, travel_ba
...
def plan_all_edges_bottom_center_first(
    ...
    Global priority (strongest to weakest):
      1) lower z_min first
      2) lower z_max first
      3) for horizontal edges, prefer those starting exactly at current tip vertex
    ...
    while remaining:
        ...
        for ei in remaining:
            ...
            dz_edge = float(plan.p1_tip[2] - plan.p0_tip[2])
            is_horizontal = abs(dz_edge) <= float(horizontal_z_tol_mm)
            starts_at_current = float(travel_to_start) <= float(endpoint_snap_tol_mm)
            connected_horizontal_rank = 0 if (is_horizontal and starts_at_current) else 1
