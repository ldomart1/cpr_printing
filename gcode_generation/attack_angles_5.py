...
Generate Duet/RRF-friendly G-code for:
  ...
  2) The original six-plane XZ pattern from the provided script
  3) Six additional planes with:
       - continuous inverted horizontal triangles in XZ
       - separated vertical-triangle half-figures in XZ
...
Behavior summary
----------------
- Preserves the original six planes from the uploaded script.
- Keeps the print organized by extrusion-multiplier groups of four printed planes.
- Keeps a uniform 5 mm Y offset between consecutive figure planes in each group.
- Plane 3 is shifted down by 15 mm in Z from the prior added-horizontal placement.
- Plane 4 follows the exact top-down two-subset construction requested by the user.
- Plane 2 uses the original shifted lower subset and an exact upper-right subset into the fixed corner at x=100, z=-155.
- All figure geometry is placed inside X=[40,100] and Z=[-185,-155] by default.
- The new horizontal triangles:
    * mirror the original step1 roof geometry across the horizontal base line
    * stay in the XZ plane at constant Y
    * keep the plane-3 placement used in this revision
    * start at the most +X tip, print the diagonal triangle passes first,
      and finish with the mirrored horizontal line
- The new vertical triangles:
    * stay in the XZ plane at constant Y
    * start from the top point and extrude downward
    * use the exact two-subset top-down construction requested by the user
    * offset both half-figures in X so their combined footprint occupies the
      same overall X-span as the original 60 mm-wide figure
    * use a non-extruding -X, +Z, +X, -Z travel after each printed line

Coordinate convention
---------------------
- All original and additional figures are described in the XZ plane.
...
DEFAULT_STEP2_STRAIGHT_EXTEND_Z_REL = 30.0
DEFAULT_TOP_HORIZONTAL_LEN = 5.0
DEFAULT_NEW_HORIZONTAL_Z_SHIFT = 45.0
DEFAULT_NEW_VERTICAL_X_SPACING = -45.0
DEFAULT_FIRST_ADDED_PLANE_GAP = 0.0
DEFAULT_PLANE3_EXTRA_Z_OFFSET = -15.0
DEFAULT_PLANE4_Z_OFFSET = 0.0
DEFAULT_PLANE2_LOWER_X_SHIFT = 30.0
DEFAULT_PLANE2_UPPER_X_SHIFT = -30.0
DEFAULT_PLANE2_UPPER_Z_SHIFT = 0.0
...
class PolylinePass:
    """
    Continuous tip-space print path made of connected straight segments.
    """
    points_tip: List[np.ndarray]
    label: str


def shift_point_tip(p: np.ndarray, dx: float = 0.0, dy: float = 0.0, dz: float = 0.0) -> np.ndarray:
    return np.asarray(p, dtype=float).copy() + np.array([float(dx), float(dy), float(dz)], dtype=float)


def shift_segment_tip(seg: Segment, dx: float = 0.0, dy: float = 0.0, dz: float = 0.0, label_suffix: str = "") -> Segment:
    return Segment(
        shift_point_tip(seg.p0_tip, dx=dx, dy=dy, dz=dz),
        shift_point_tip(seg.p1_tip, dx=dx, dy=dy, dz=dz),
        f"{seg.label}{label_suffix}",
    )
...
def _intersect_two_rays_xz(
    ...
    """
    Intersect two rays in the XZ plane:
      ray 1 starts at p_left  with direction angle_left_deg
      ray 2 starts at p_right with direction angle_right_deg

    Angles are measured in the standard XZ convention used elsewhere in the script:
    0 deg = +X, 90 deg = +Z, -90 deg = -Z, +/-180 deg = -X.

    Returns the intersection point as a tip-space XYZ point using the common Y plane.
    Raises if the rays are parallel or if the forward-ray intersection is invalid.
    """
    x1, y1, z1 = [float(v) for v in p_left]
    x2, y2, z2 = [float(v) for v in p_right]

    th1 = math.radians(float(angle_left_deg))
    th2 = math.radians(float(angle_right_deg))

    d1 = np.array([math.cos(th1), math.sin(th1)], dtype=float)   # [dx, dz]
    d2 = np.array([math.cos(th2), math.sin(th2)], dtype=float)

    A = np.array([
        [d1[0], -d2[0]],
        [d1[1], -d2[1]],
    ], dtype=float)
    b = np.array([x2 - x1, z2 - z1], dtype=float)

    det = float(np.linalg.det(A))
    if abs(det) < 1e-12:
        raise ValueError(
            f"Rays are parallel or nearly parallel: left={angle_left_deg} deg, right={angle_right_deg} deg"
        )

    t1, t2 = np.linalg.solve(A, b)
    if t1 < -1e-9 or t2 < -1e-9:
        raise ValueError(
            f"Ray intersection lies behind one of the tips: left={angle_left_deg} deg, right={angle_right_deg} deg"
        )

    return p_xyz(x1 + t1 * d1[0], y1, z1 + t1 * d1[1])
...
def make_step1_segments(start_tip: np.ndarray, y_plane: float,
    ...
    """
    Original step-1 geometry from the provided script.
    """
...
def make_additional_horizontal_triangle_passes(
    ...
    """
    Additional horizontal-triangle plane in XZ.

    This is the original step1 roof geometry mirrored across the horizontal
    base line and shifted upward by z_shift in Z. Each mirrored triangle is
    written continuously from the most +X tip to the opposite tip:
        right tip -> mirrored apex -> left tip
    The final pass is the mirrored horizontal base line written from +X to -X.
    """
    sx, _, sz = [float(v) for v in start_tip]
    base_z = sz + float(z_shift)

    left_tip = p_xyz(sx, y_plane, base_z)
    right_tip = p_xyz(sx + line_len, y_plane, base_z)

    passes: List[PolylinePass] = []
    angles = np.arange(float(angle_max), float(angle_step) - 1e-9, -float(angle_step))
    for a in angles:
        z_mid = base_z - math.tan(math.radians(float(a))) * float(mid_x)
        p_mid = p_xyz(sx + float(mid_x), y_plane, z_mid)
        passes.append(
            PolylinePass(
                points_tip=[right_tip.copy(), p_mid.copy(), left_tip.copy()],
                label=f"add_step1_mirror_a{a:.0f}",
            )
        )

    passes.append(
        PolylinePass(
            points_tip=[right_tip.copy(), left_tip.copy()],
            label="add_step1_mirror_horizontal_0deg",
        )
    )
    return passes


def make_additional_vertical_triangle_segments_xz(
    start_tip: np.ndarray,
    y_plane: float,
    line_len: float,
    z_target_rel: float,
    ...
    """
    Additional vertical-triangle half-figures in the XZ plane (constant Y).

    These are packed into the same overall horizontal limits as plane 2:
    X=[sx, sx+line_len]. Both halves share the same central top point at
    x=sx+line_len/2 and extrude downward to the base line z=sz, so the left-
    half lines occupy [sx, sx+line_len/2] and the right-half lines occupy
    [sx+line_len/2, sx+line_len]. The second-half figure is printed first.
    Every printed segment starts at the top point and extrudes downward.
    """
    sx, _, sz = [float(v) for v in start_tip]
    x_mid = sx + 0.5 * float(line_len)
    z_drop = float(straight_extend_z_rel)

    origin_common = p_xyz(x_mid, y_plane, sz)
    top_common = p_xyz(x_mid, y_plane, sz + z_drop)

    second_half_segments: List[Segment] = []
    second_half_segments.append(Segment(top_common.copy(), origin_common.copy(), "add_step2_second_vertical"))
    for ang in range(85, 40, -5):
        dx = z_drop / math.tan(math.radians(float(ang)))
        p_end = p_xyz(x_mid + dx, y_plane, sz)
        second_half_segments.append(Segment(top_common.copy(), p_end, f"add_step2_second_top_to_{ang}deg"))

    first_half_segments: List[Segment] = []
    for ang in range(95, 140, 5):
        dx = z_drop / math.tan(math.radians(float(180 - ang)))
        p_end = p_xyz(x_mid - dx, y_plane, sz)
        first_half_segments.append(Segment(top_common.copy(), p_end, f"add_step2_first_top_to_{ang}deg"))

    return {
        "second_half_segments": second_half_segments,
        "first_half_segments": first_half_segments,
        "top_second": top_common,
        "top_first": top_common.copy(),
        "origin_second": origin_common,
        "origin_first": origin_common.copy(),
    }
...
def make_step2_segments(
    ...
    """
    Original step-2 geometry from the provided script.
...
def make_plane2_second_subset_segments(
    start_step1_tip: np.ndarray,
    y_plane: float,
    line_len: float,
    z_drop: float,
    ...
    """
    Exact plane-2 second subset requested by the user.

    With the default placement this produces:
      - fixed triangle corner at (100, -155)
      - first line from (100, -185) to (100, -155)
      - subsequent base starts on z=-185 at 5-degree slope increments
        down to the last start point (70, -185), all ending at (100, -155)
    """
    sx, _, sz = [float(v) for v in start_step1_tip]
    corner = p_xyz(sx + float(line_len), y_plane, sz + float(z_drop))
    base_z = sz

    segs: List[Segment] = []
    for ang in range(90, 40, -5):
        if ang == 90:
            x0 = sx + float(line_len)
        else:
            x0 = sx + float(line_len) - float(z_drop) / math.tan(math.radians(float(ang)))
        p0 = p_xyz(x0, y_plane, base_z)
        segs.append(Segment(p0, corner.copy(), f"plane2_subset2_{ang}deg"))

    return segs, corner


def make_plane4_exact_subsets(
    start_tip: np.ndarray,
    y_plane: float,
    mid_x: float,
    z_drop: float,
    ...
    """
    Exact plane-4 geometry requested by the user.

    With the default placement this produces:
      top start point = (70, -155)

      subset 1:
        vertical first line to (70, -185), then 5-degree increments leftward
        until the last point (40, -185)

      subset 2:
        return to the same top start point, skip the vertical line, and draw
        the right-hand fan in 5-degree increments until the last point
        (100, -185)
    """
    sx, _, sz = [float(v) for v in start_tip]
    top = p_xyz(sx + float(mid_x), y_plane, sz + float(z_drop))
    base_z = sz

    subset1: List[Segment] = []
    for ang in range(90, 40, -5):
        if ang == 90:
            x_end = sx + float(mid_x)
        else:
            x_end = sx + float(mid_x) - float(z_drop) / math.tan(math.radians(float(ang)))
        subset1.append(Segment(top.copy(), p_xyz(x_end, y_plane, base_z), f"plane4_subset1_{ang}deg"))

    subset2: List[Segment] = []
    for ang in range(85, 40, -5):
        x_end = sx + float(mid_x) + float(z_drop) / math.tan(math.radians(float(ang)))
        subset2.append(Segment(top.copy(), p_xyz(x_end, y_plane, base_z), f"plane4_subset2_{ang}deg"))
...
class CalibratedPatternWriter:
    ...
    def safe_travel_minus_x_plus_z_plus_x_minus_z_tip(
        self,
        ...
        side_x = min(float(cur[0]), float(target_stage[0])) - abs(float(x_clearance))

        move_neg_x = cur.copy()
        move_neg_x[0] = side_x
        self.move_stage_xyzbc(move_neg_x, feed=self.travel_feed, comment=f"{comment_prefix}: -X")

        move_up = move_neg_x.copy()
        move_up[2] = move_neg_x[2] + float(lift_dz_stage)
        self.move_stage_xyzbc(move_up, feed=self.travel_feed, comment=f"{comment_prefix}: +Z")

        move_pos_x = move_up.copy()
        move_pos_x[0] = target_stage[0]
        move_pos_x[1] = target_stage[1]
        self.move_stage_xyzbc(move_pos_x, feed=self.travel_feed, comment=f"{comment_prefix}: +X")

        move_down = move_pos_x.copy()
        move_down[2] = target_stage[2]
        self.move_stage_xyzbc(move_down, feed=self.travel_feed, comment=f"{comment_prefix}: -Z")

    def safe_travel_plus_x_plus_z_minus_x_minus_z_tip(
        self,
        target_tip: np.ndarray,
        x_clearance: float,
        lift_dz_stage: float,
        ...
        side_x = max(float(cur[0]), float(target_stage[0])) + abs(float(x_clearance))

        move_pos_x = cur.copy()
        move_pos_x[0] = side_x
        self.move_stage_xyzbc(move_pos_x, feed=self.travel_feed, comment=f"{comment_prefix}: +X")

        move_up = move_pos_x.copy()
        move_up[2] = move_pos_x[2] + float(lift_dz_stage)
        self.move_stage_xyzbc(move_up, feed=self.travel_feed, comment=f"{comment_prefix}: +Z")

        move_neg_x = move_up.copy()
        move_neg_x[0] = target_stage[0]
        move_neg_x[1] = target_stage[1]
        self.move_stage_xyzbc(move_neg_x, feed=self.travel_feed, comment=f"{comment_prefix}: -X")

        move_down = move_neg_x.copy()
        move_down[2] = target_stage[2]
        self.move_stage_xyzbc(move_down, feed=self.travel_feed, comment=f"{comment_prefix}: -Z")
    ...
    def print_tip_polyline(self, pts_tip: List[np.ndarray], label: str, extrusion_multiplier: float):
        pts = [np.asarray(p, dtype=float).copy() for p in pts_tip]
        if len(pts) < 2:
            raise ValueError("Polyline print pass requires at least two points.")

        p0_stage = self.tip_to_stage(pts[0])
        if self.cur_stage_xyz is None or np.linalg.norm(self.cur_stage_xyz - self._clamp_stage(p0_stage, "compare polyline p0")) > 1e-6:
            self.move_stage_xyzbc(p0_stage, feed=self.travel_feed, comment=f"{label}: move to print start")

        self.pressure_preload_before_print()
        self.f.write(f"; print {label} (continuous tip-space polyline, fixed B/C)\n")

        last_stage = self.cur_stage_xyz.copy()
        for i in range(len(pts) - 1):
            p0_tip = pts[i]
            p1_tip = pts[i + 1]
            if np.linalg.norm(p1_tip - p0_tip) <= 1e-12:
                continue
            ...
            for j in range(1, len(ts)):
                ...
                if np.linalg.norm((p_stage + self.tip_offset_fixed) - p_tip) > 1e-8:
                    raise RuntimeError(f"Tip-tracking consistency failed in {label} polyline segment {i+1}.{j}.")

                pc = self._clamp_stage(p_stage, f"{label} polyline seg {i+1}.{j}")
                axes = [
                    (self.cal.x_axis, pc[0]),
                    (self.cal.y_axis, pc[1]),
                    (self.cal.z_axis, pc[2]),
...
def write_pattern_gcode(
    ...
    node_dwell_ms: int,
    bbox: dict,
    new_horizontal_z_shift: float,
    new_vertical_x_spacing: float,
):
    ...
    with open(out_path, "w") as f:
        ...
        f.write("; main pattern: grouped XZ-plane original + additional triangle tests in Y-offset planes\n")
        ...
        f.write(f"; plane 3 base horizontal z shift = {float(new_horizontal_z_shift):.3f} mm; extra plane-3 z offset = {DEFAULT_PLANE3_EXTRA_Z_OFFSET:.3f} mm\n")
        f.write(f"; plane 4 z offset = {DEFAULT_PLANE4_Z_OFFSET:.3f} mm\n")
        f.write(f"; plane 2 subset 1 shift = [+{DEFAULT_PLANE2_LOWER_X_SHIFT:.3f} X, +0.000 Z] mm\n")
        f.write("; plane 2 subset 2 is the explicit fan into the fixed corner at x=100, z=-155 (with default placement)\n")
        f.write("; plane 4 uses the explicit top-down two-subset construction from x=70, z=-155 (with default placement)\n")
        f.write("; design envelope for all four figure planes per group: X in [40, 100], Z in [-185, -155]\n")
        f.write(f"; plane-4 travel clearance parameter derived from legacy x-shift arg = {float(new_vertical_x_spacing):.3f} mm\n")
        ...
        for i, seg in enumerate(prep_line_segments, start=1):
            g.move_to_tip(seg.p0_tip, feed=travel_feed, comment=f"prep line {i} direct move to start (no Z jump travel)")
        ...
        for rep_idx, ex_mult in enumerate(DEFAULT_EXTRUSION_MULTIPLIERS, start=1):
            ...
            g.safe_travel_z_up_x_then_z_down_tip(
                step1_start_tip, lift_dz_stage=DEFAULT_RETURN_LIFT_Z,
                comment_prefix=f"group rep{rep_idx} plane1 return after horizontal"
            )
            ...
            for k in range(0, len(roof_parts), 2):
                ...
                g.safe_travel_z_up_x_then_z_down_tip(
                    step1_start_tip, lift_dz_stage=DEFAULT_RETURN_LIFT_Z,
                    comment_prefix=f"group rep{rep_idx} plane1 return after roof pair {k//2 + 1}"
                )

            g.print_tip_segment(vertical, extrusion_multiplier=ex_mult)
            total_print_passes += 1

            # ---- Plane 2: original step2 lower subset + exact user-specified upper subset ----
            ...
            famA_no90 = [s for s in famA if s.label != "step2_famA_90deg_extended"]

            lower_dx = DEFAULT_PLANE2_LOWER_X_SHIFT
            lower_dz = 0.0
            shifted_famA_no90 = [shift_segment_tip(seg, dx=lower_dx, dz=lower_dz, label_suffix="_lower_shifted") for seg in famA_no90]
            shifted_step2_origin_tip = shift_point_tip(step2_origin_tip, dx=lower_dx, dz=lower_dz)

            g.safe_travel_x_first_then_down_z_tip(
                shifted_step2_origin_tip,
                comment_prefix=f"group rep{rep_idx} plane2 subset1 move to shifted origin (x first then z)"
            )

            for i, seg in enumerate(shifted_famA_no90):
                ...
                if np.linalg.norm(g.tip_to_stage(seg.p0_tip) - g.cur_stage_xyz) > 1e-6:
                    g.safe_travel_x_first_then_down_z_tip(
                        seg.p0_tip,
                        comment_prefix=f"group rep{rep_idx} plane2 subset1 line {i+1} travel to start"
                    )
                g.print_tip_segment(seg, extrusion_multiplier=ex_mult)
                total_print_passes += 1

                if i < len(shifted_famA_no90) - 1:
                    g.safe_travel_x_first_then_down_z_tip(
                        shifted_step2_origin_tip,
                        comment_prefix=f"group rep{rep_idx} plane2 subset1 return to shifted origin"
                    )

            plane2_subset2_segments, plane2_subset2_corner = make_plane2_second_subset_segments(
                start_step1_tip=step1_start_tip,
                y_plane=y_step2,
                line_len=DEFAULT_LINE_LENGTH,
                z_drop=DEFAULT_STEP2_STRAIGHT_EXTEND_Z_REL,
            )
            plane2_subset2_segments = [
                shift_segment_tip(seg, dx=DEFAULT_PLANE2_UPPER_X_SHIFT, label_suffix="_shifted")
                for seg in plane2_subset2_segments
            ]

            for i, seg in enumerate(plane2_subset2_segments, start=1):
                if i == 1:
                    g.safe_travel_x_first_then_down_z_tip(
                        seg.p0_tip,
                        comment_prefix=f"group rep{rep_idx} plane2 subset2 move to first base start"
                    )
                else:
                    g.safe_travel_x_first_then_down_z_tip(
                        seg.p0_tip,
                        comment_prefix=f"group rep{rep_idx} plane2 subset2 -X then -Z to next base start"
                    )
                g.print_tip_segment(seg, extrusion_multiplier=ex_mult)
                total_print_passes += 1

            # ---- Plane 3: additional horizontal triangles ----
            y_add_h = y_step2 + DEFAULT_PLANE_Y_STEP + DEFAULT_FIRST_ADDED_PLANE_GAP
            add_start_tip = p_xyz(sx, y_add_h, sz + DEFAULT_PLANE3_EXTRA_Z_OFFSET)
            horiz_passes = make_additional_horizontal_triangle_passes(
                start_tip=add_start_tip,
                y_plane=y_add_h,
                line_len=DEFAULT_LINE_LENGTH,
                mid_x=DEFAULT_MID_X,
                angle_step=DEFAULT_ANGLE_STEP,
                angle_max=DEFAULT_ANGLE_MAX_1,
                z_shift=float(new_horizontal_z_shift),
            )

            if horiz_passes:
                g.safe_travel_z_up_x_then_z_down_tip(
                    horiz_passes[0].points_tip[0],
                    lift_dz_stage=DEFAULT_RETURN_LIFT_Z,
                    comment_prefix=f"group rep{rep_idx} plane3 mirrored-horizontal move to first +X tip"
                )

            for i, pp in enumerate(horiz_passes, start=1):
                if g.cur_stage_xyz is None or np.linalg.norm(g.tip_to_stage(pp.points_tip[0]) - g.cur_stage_xyz) > 1e-6:
                    g.safe_travel_z_up_x_then_z_down_tip(
                        pp.points_tip[0],
                        lift_dz_stage=DEFAULT_RETURN_LIFT_Z,
                        comment_prefix=f"group rep{rep_idx} plane3 mirrored-horizontal pass {i} move to start"
                    )
                g.print_tip_polyline(pp.points_tip, pp.label, extrusion_multiplier=ex_mult)
                total_print_passes += 1

            # ---- Plane 4: exact user-specified vertical triangle subsets in XZ ----
            y_add_v = y_add_h + DEFAULT_PLANE_Y_STEP
            plane4_pack = make_plane4_exact_subsets(
                start_tip=p_xyz(sx, y_add_v, sz + DEFAULT_PLANE4_Z_OFFSET),
                y_plane=y_add_v,
                mid_x=DEFAULT_MID_X,
                z_drop=DEFAULT_STEP2_STRAIGHT_EXTEND_Z_REL,
            )

            plane4_x_clear = max(5.0, abs(float(new_vertical_x_spacing)) * 0.20)

            g.safe_travel_x_first_then_down_z_tip(
                plane4_pack["top"],
                comment_prefix=f"group rep{rep_idx} plane4 subset1 move to common top start"
            )
            for i, seg in enumerate(plane4_pack["subset1"], start=1):
                if g.cur_stage_xyz is None or np.linalg.norm(g.tip_to_stage(seg.p0_tip) - g.cur_stage_xyz) > 1e-6:
                    g.safe_travel_x_first_then_down_z_tip(
                        seg.p0_tip,
                        comment_prefix=f"group rep{rep_idx} plane4 subset1 line {i} move to top start"
                    )
                g.print_tip_segment(seg, extrusion_multiplier=ex_mult)
                total_print_passes += 1
                if i < len(plane4_pack["subset1"]):
                    g.safe_travel_minus_x_plus_z_plus_x_minus_z_tip(
                        plane4_pack["top"],
                        x_clearance=plane4_x_clear,
                        lift_dz_stage=DEFAULT_RETURN_LIFT_Z,
                        comment_prefix=f"group rep{rep_idx} plane4 subset1 line {i} return to top"
                    )

            g.safe_travel_plus_x_plus_z_minus_x_minus_z_tip(
                plane4_pack["top"],
                x_clearance=plane4_x_clear,
                lift_dz_stage=DEFAULT_RETURN_LIFT_Z,
                comment_prefix=f"group rep{rep_idx} plane4 subset2 return to common top start"
            )
            for i, seg in enumerate(plane4_pack["subset2"], start=1):
                if g.cur_stage_xyz is None or np.linalg.norm(g.tip_to_stage(seg.p0_tip) - g.cur_stage_xyz) > 1e-6:
                    g.safe_travel_plus_x_plus_z_minus_x_minus_z_tip(
                        seg.p0_tip,
                        x_clearance=plane4_x_clear,
                        lift_dz_stage=DEFAULT_RETURN_LIFT_Z,
                        comment_prefix=f"group rep{rep_idx} plane4 subset2 line {i} move to top start"
                    )
                g.print_tip_segment(seg, extrusion_multiplier=ex_mult)
                total_print_passes += 1
                if i < len(plane4_pack["subset2"]):
                    g.safe_travel_plus_x_plus_z_minus_x_minus_z_tip(
                        plane4_pack["top"],
                        x_clearance=plane4_x_clear,
                        lift_dz_stage=DEFAULT_RETURN_LIFT_Z,
                        comment_prefix=f"group rep{rep_idx} plane4 subset2 line {i} return to top"
                    )

            y_cursor = y_add_v + DEFAULT_PLANE_Y_STEP
...
def main(args):
    ...
    write_pattern_gcode(
        ...
        bbox=bbox,
        new_horizontal_z_shift=float(args.new_horizontal_z_shift),
        new_vertical_x_spacing=float(args.new_vertical_x_spacing),
    )
...
if __name__ == "__main__":
    ...
    ap.add_argument("--safe-approach-z", type=float, default=DEFAULT_SAFE_APPROACH_Z,
                    help="Safe Z used before startup/end positioning.")
    ...
    ap.add_argument("--new-horizontal-z-shift", type=float, default=DEFAULT_NEW_HORIZONTAL_Z_SHIFT,
                    help="Z shift applied to the additional horizontal triangle planes. Default chosen so all figure planes stay within Z=[-185,-125].")
    ap.add_argument("--new-vertical-x-spacing", type=float, default=DEFAULT_NEW_VERTICAL_X_SPACING,
                    help="Legacy compatibility option. The updated vertical-half geometry auto-packs both halves into the original X-span, so this value is ignored.")
