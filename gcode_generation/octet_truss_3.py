...
DEFAULT_ORIGIN_Z = -140.0

# Geometry
DEFAULT_ORDER = 1                 # number of octet unit cells along cube edge
DEFAULT_PITCH_MM = 14.0           # octet strut length (FCC nearest-neighbor distance)
...
def wrap_deg_to_pm180(angle_deg: float) -> float:
...
def choose_equivalent_c_in_window(
    ...
    c_min_deg: float = -360.0,
    c_max_deg: float = 360.0,
) -> float:
    """
    Return an equivalent angle target_c + 360*k that:
      1) lies inside [c_min_deg, c_max_deg], if possible
      2) is nearest ref_c_deg (or nearest 0 if ref is None)

    If no equivalent lies inside the window, return the clipped nearest equivalent.
    """
...
def write_gcode_octet_truss(
    ...
    def emit_tip_fixed_reorient_via_neutral(
        ...
        cur_b = float(b_from)
        cur_c = float(c_from)
        target_c = choose_equivalent_c_in_window(
            target_c_deg=float(c_to),
            ref_c_deg=float(c_from),
            c_min_deg=float(c_window_min),
            c_max_deg=float(c_window_max),
        )
    ...
    def recenter_c_if_needed(
        ...
        if abs(desired_c - float(cur_c)) <= 1e-9:
            return float(cur_b), float(cur_c)

        if abs(float(cur_b) - float(neutral_b_cmd)) > 1e-9:
            _, cur_b, cur_c = emit_tip_tracked_segmented_move(
                fobj,
                tip_point, tip_point,
                float(cur_b), float(neutral_b_cmd),
                float(cur_c), float(cur_c),
                samples=travel_samples,
                feed=float(travel_feed),
                comment="; C recenter: uncurl B to neutral",
                context_prefix="c recenter uncurl",
            )

        _, cur_b, cur_c = emit_tip_tracked_segmented_move(
            fobj,
            tip_point, tip_point,
            float(cur_b), float(cur_b),
            float(cur_c), float(desired_c),
            samples=travel_samples,
            feed=float(travel_feed),
            comment="; C recenter: rotate C to equivalent angle in bounded range",
            context_prefix="c recenter rotate",
        )
        return cur_b, cur_c
    ...
    with open(out_path, 'w') as f:
        ...
        f.write('; same-tip turns are executed as: B->neutral, C rotate, B->target, while holding tip fixed\n')
        f.write(f'; C command window = [{float(c_window_min):.3f}, {float(c_window_max):.3f}] deg, recenter margin = {float(c_recenter_margin_deg):.3f} deg\n')
        ...
        for ei, plan in enumerate(plans):
            ...
            if first_edge:
                ...
                if same_tip_start:
                    need_recenter = (
                        float(current_c) < (float(c_window_min) + float(c_recenter_margin_deg))
                        or float(current_c) > (float(c_window_max) - float(c_recenter_margin_deg))
                    )
                    if need_recenter:
                        current_b, current_c = recenter_c_if_needed(
                            f,
                            tip_point=p0_tip,
                    ...
                    if seg_idx + 1 < len(waypoints):
                        ...
                        if float(np.linalg.norm(bxy[:2] - a[:2])) > 1e-9:
                            ...
                            _, current_b, current_c = emit_tip_tracked_segmented_move(
                                f, a, bxy, float(current_b), float(current_b), float(current_c), float(target_c),
                                samples=travel_samples,
                                feed=float(travel_feed),
                    ...
                    if not has_xy_leg and abs(float(current_c) - float(c)) > 1e-9:
                        ...
                        _, current_b, current_c = emit_tip_tracked_segmented_move(
                            f, current_tip, current_tip, float(current_b), float(current_b), float(current_c), float(target_c),
                            samples=travel_samples,
                            feed=float(travel_feed),
...
def main(args):
    ...
    if float(args.c_window_min) > float(args.c_window_max):
        args.c_window_min, args.c_window_max = float(args.c_window_max), float(args.c_window_min)
    ...
    print(f"C command window: [{float(args.c_window_min):.3f}, {float(args.c_window_max):.3f}] deg")
    print(f"C recenter margin: {float(args.c_recenter_margin_deg):.3f} deg")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--c-window-min", type=float, default=DEFAULT_C_WINDOW_MIN,
                    help="Lower bound for commanded C angle window.")
    ap.add_argument("--c-window-max", type=float, default=DEFAULT_C_WINDOW_MAX,
                    help="Upper bound for commanded C angle window.")
    ap.add_argument("--c-recenter-margin-deg", type=float, default=DEFAULT_C_RECENTER_MARGIN_DEG,
                    help="Recenters C during safe travel when within this margin of the C window edge.")
